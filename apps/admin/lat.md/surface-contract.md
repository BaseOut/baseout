...
`apps/admin`'s public surface is **only what's needed for staff workflows**. Outbound calls into `apps/server` use the same `SERVER_INTERNAL_TOKEN` mechanism that `apps/web` uses.
...
The token used is the same `SERVER_INTERNAL_TOKEN` shared with `apps/web` — see [root cross-app-comm](../../../lat.md/cross-app-comm.md). Rotation requires updating all three Worker Secret namespaces.
