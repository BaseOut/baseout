#!/usr/bin/env node
// COPIED from apps/web/scripts/sync-secrets.mjs — that file is canonical;
// keep this one identical apart from this header.
//
// Sync apps/admin/.dev.vars into the deployed Worker's secret set via
...
//
// Invoked from package.json `secrets:sync` after `wrangler deploy`. The
// fix-up exists because hand-set secrets on dev Workers drift away from
// .dev.vars; the deploy script keeps them aligned automatically. See
// CLAUDE.md §3.3 and the auto-memory.
