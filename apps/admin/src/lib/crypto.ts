/**
 * AES-256-GCM helpers for the login → admin handoff token.
 *
 * COPIED from apps/web/src/lib/crypto.ts — that file is canonical; keep this
 * one identical. Web encrypts (mints the handoff token in
 * /api/admin/handoff); admin decrypts (src/lib/handoff.ts). Both sides key
 * off ADMIN_HANDOFF_SECRET (base64, 32 bytes).
 *
