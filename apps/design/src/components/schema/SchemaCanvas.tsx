 ...
 *
 * The only React in the app: an interactive node-graph the Astro/daisyUI shell
 * can't express. Render with `client:only="react"`.
 *
 * Decisions (see overview/schema/research.html):
 * - HYBRID node — header (health dot + name + counts) over the relationship-bearing
 *   field rows (primary + linked-record fields). Full field list lives in the side
 *   panel (next increment). This keeps nodes clean but the diagram still shows WHICH
 *   fields relate — a real ER view, not just boxes.
...
interface Props {
  ...
  /** Documents the current diagram can be added to — feeds the "Add to doc" dropdown. */
...
const HEALTH: Record<SchemaTable['health'], string> = {
...
function relKind(f: SchemaField): RelKind | null {
  ...
  if (f.type === 'link' || f.linkedTableId) return 'linked';
  return null;
}
...
function RelTableNode({ data, selected }: NodeProps<Node<RelNodeData>>) {
  ...
  return (
    <div style={{ width: REL_NODE_W, background: 'var(--color-base-100)', border: `1px solid ${selected ? 'var(--color-primary)' : 'var(--color-base-300)'}`, borderRadius: 10, boxShadow: selected ? '0 0 0 1px var(--color-primary), 0 8px 24px oklch(0 0 0 / 0.18)' : '0 1px 2px oklch(0 0 0 / 0.06)', display: 'flex', alignItems: 'center', gap: 8, height: REL_NODE_H, padding: '0 12px' }}>
      ...
      <span aria-hidden style={{ width: 9, height: 9, borderRadius: 999, flex: 'none', background: HEALTH[table.health] }} />
...
function Canvas({ tables, bases, baseHealth, docs, genState = 'ready', embed = false, relationships = EMPTY_REL, automations = EMPTY_AUTO, interfaces = EMPTY_IF }: Props) {
  ...
  // "Add to doc" dropdown state: the picked target ('__new' = create a new doc) + the typed
  // name for a new doc. Search within the (possibly long) doc list; the list scrolls past ~5.
  ...
  const filteredDocs = useMemo(() => {
    ...
    return q ? docList.filter((d) => d.title.toLowerCase().includes(q)) : docList;
  }, [docList, docQuery]);
  // Top-right success toast (the documented daisyUI `toast toast-top toast-end` pattern).
  ...
  // Add the diagram to the chosen doc — or create + add a NAMED new doc. Confirms via a
  // top-right toast and closes the dropdown; no navigation (the user stays in Visualize).
  const confirmAddToDoc = () => {
    const isNew = addTarget === '__new';
    const name = isNew ? newDocName.trim() : (docList.find((d) => d.id === addTarget)?.title ?? 'the document');
    if (!addTarget || (isNew && !name)) return;
    showToast(isNew ? `“${name}” created — diagram added` : `Diagram added to “${name}”`);
  ...
  const tableGroups = useMemo(() => {
    const mk = (t: SchemaTable): FacetItem => ({ id: t.id, label: t.name, dot: HEALTH[t.health] });
  ...
  const toggleSet = (set: Dispatch<SetStateAction<Set<string>>>) => (id: string) =>
  ...
  useEffect(() => {
    ...
    const l = mode === 'relationships'
      ? relLayout(visibleTables, relationships, { hiddenRelTypes })
      : mode === 'appLayer'
  ...
  }, [visibleTables, hiddenTypes, hiddenFields, hiddenRel, hiddenRelTypes, hiddenAppKinds, includeRemovedApp, expandFields, mode, relationships, automations, interfaces, setNodes, setEdges, rf]);
  ...
  const minimapColor = useCallback(
    (n: Node) => (n.type === 'baseGroup' ? 'transparent' : HEALTH[(n.data as TableNodeData)?.table?.health ?? 'green'] ?? 'var(--color-base-300)'),
    [],
  );
  ...
  const onNodeClick: NodeMouseHandler = useCallback((_, node) => {
    ...
    // App-layer entity nodes open their panel; the table/field substrate opens the shared entity
    // panel (field nodes carry an `fld:` id prefix).
    if (node.type === 'appEntity') { openAppEntity((node.data as AppEntityData)?.kind, node.id); return; }
    const id = node.id.startsWith('fld:') ? node.id.slice(4) : node.id;
  ...
  const legendCols = useMemo<LegendCol[] | null>(() => {
    if (mode === 'data') {
      return [
        { title: 'Health', items: HEALTH_LEGEND },
  ...
  const showLegend = !embed && (mode === 'data' ? visibleTables.length > 0 : mode === 'relationships' ? relationships.length > 0 : hasApp);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%' }}>
      {/* Success toast — pinned top-right (daisyUI toast pattern), auto-dismisses. */}
      {toastMsg && (
        <div className="toast toast-top toast-end" style={{ zIndex: 60 }}>
          <div className="alert alert-success" role="status" aria-live="polite">
            <span aria-hidden style={{ width: 16, height: 16, display: 'grid', placeItems: 'center' }}><CheckGlyph /></span>
            <span>{toastMsg}</span>
          </div>
        </div>
      )}
      {/* Toolbar — the shared .sch-tb pattern: search │ faceted Display filters … → Add-to-doc + Export. */}
      {!embed && (
      <div className="sch-tb" style={{ padding: '8px 12px', borderBottom: '1px solid var(--color-base-200)' }}>
              ...
              {matches.map((t) => (
                <li key={t.id}>
                  <a onClick={() => jumpTo(t.id)} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span aria-hidden style={{ width: 7, height: 7, borderRadius: 999, flex: 'none', background: HEALTH[t.health] }} />
        ...
        {/* Add the current diagram to a document — a small contextual dropdown (doc list + confirm). */}
        {docs && docs.length > 0 && (
          <div className="dropdown dropdown-end">
            <div tabIndex={0} role="button" className="btn btn-sm btn-neutral gap-1">
              <span aria-hidden style={{ fontSize: 15, lineHeight: 1, marginTop: -1 }}>+</span>Add to doc
            </div>
            <div tabIndex={0} className="dropdown-content bg-base-100 rounded-box border border-base-300 shadow-lg" style={{ zIndex: 30, marginTop: 4, width: 266, padding: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.04em', textTransform: 'uppercase', opacity: 0.5, padding: '2px 4px 8px' }}>Add this diagram to</div>
              {/* Search — surfaces once the list is long enough to scan (Dan: "could be a very long list"). */}
              {docList.length > 5 && (
                <label className="input input-sm" style={{ width: '100%', marginBottom: 6 }}>
                  <span aria-hidden style={{ width: 13, height: 13, display: 'grid', placeItems: 'center', opacity: 0.5 }}><SearchGlyph /></span>
                  <input type="text" placeholder="Search documents" aria-label="Search documents" value={docQuery} onChange={(e) => setDocQuery(e.target.value)} autoComplete="off" />
                </label>
              )}
              {/* Existing docs — capped to ~5 rows; scrolls past that (the scrollbar Dan asked for). */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, maxHeight: 178, overflowY: 'auto' }}>
                {filteredDocs.map((d) => (
                  <label key={d.id} className="hover:bg-base-200" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px', borderRadius: 7, cursor: 'pointer' }}>
                    <input type="radio" name="add-to-doc" className="radio radio-xs radio-primary" style={{ flex: 'none' }} checked={addTarget === d.id} onChange={() => { setAddTarget(d.id); setNewDocName(''); }} />
                    <span aria-hidden style={{ width: 15, height: 15, display: 'grid', placeItems: 'center', opacity: 0.6 }}><FileGlyph /></span>
                    <span style={{ flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13, fontWeight: 500 }}>{d.title}</span>
                  </label>
                ))}
                {filteredDocs.length === 0 && (
                  <p style={{ fontSize: 12.5, opacity: 0.55, padding: '8px 4px' }}>No documents match.</p>
                )}
              </div>
              {/* New document — pinned below the scroll area; selecting it swaps the label for an
                  inline name field (no modal, no navigation). */}
              <div className="hover:bg-base-200" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px', borderRadius: 7, color: 'var(--color-primary)', marginTop: 2 }}>
                <input type="radio" id="add-to-doc-new" name="add-to-doc" className="radio radio-xs radio-primary" style={{ flex: 'none' }} checked={addTarget === '__new'} onChange={() => { setAddTarget('__new'); setDocQuery(''); }} />
                {addTarget === '__new' ? (
                  <input
                    type="text"
                    autoFocus
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') confirmAddToDoc(); }}
                    placeholder="New document"
                    aria-label="New document name"
                    className="input input-sm"
                    style={{ flex: 1, minWidth: 0, height: 28, fontSize: 13 }}
                  />
                ) : (
                  <>
                    <span aria-hidden style={{ width: 15, textAlign: 'center', fontSize: 14, lineHeight: 1 }}>+</span>
                    <label htmlFor="add-to-doc-new" style={{ flex: 1, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>New document</label>
                  </>
                )}
              </div>
              <button type="button" className="btn btn-sm btn-primary gap-1.5" style={{ width: '100%', marginTop: 8 }}
                disabled={!addTarget || (addTarget === '__new' && !newDocName.trim())}
                onClick={confirmAddToDoc}>
                <span aria-hidden style={{ width: 13, height: 13, display: 'grid', placeItems: 'center' }}><CheckGlyph /></span>Add to document
              </button>
            </div>
          </div>
        )}
...
export default function SchemaCanvas({ tables, bases, baseHealth, docs, genState, embed, relationships, automations, interfaces }: Props) {
  return (
    <ReactFlowProvider>
      <Canvas tables={tables} bases={bases} baseHealth={baseHealth} docs={docs} genState={genState} embed={embed} relationships={relationships} automations={automations} interfaces={interfaces} />
    </ReactFlowProvider>
  );
}
...
/* — Side panel: full field list + AI "Generate description" (Pro+, 10 credits) — */
