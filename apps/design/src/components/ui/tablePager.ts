 ...
 *
 * Usage:
 *   const pager = createPager({ root, name: 'brflat', sizes: [25, 50, 100], defaultSize: 50,
 *                               storageKey: 'sch-br-pagesize', onChange: apply });
 *   // inside apply(), after filtering + sorting:
 *   const shown = pager.window(matchedRows);
...
export function pagerHtml(state: {
  ...
  const range = total === 0 ? '0' : `${(start + 1).toLocaleString()}–${Math.min(start + pageSize, total).toLocaleString()}`;
  return `
    <div class="tpg${className ? ` ${className}` : ''}">
      ...
      <div class="ml-auto text-sm text-base-content/70"><span class="mono-data" data-pager-range>${range}</span> of <span class="mono-data" data-pager-total>${total.toLocaleString()}</span></div>
      <div class="tpg-btns">
