...
<div class="grid overflow-hidden rounded-[11px] border border-base-300 bg-base-100" style="grid-template-columns:repeat(3,minmax(0,1fr));max-width:430px">
  ...
  },
  {
    ...
    description:
      'One panel, opened from two places: a <a href="/schema">Browse</a> row click and a <a href="/schema">Docs</a> entity-tag click (in reading mode). It is a right-anchored drawer over a scrim, modelled on iOS <code>UINavigationController</code>: selecting a child or related entity <strong>swaps the panel to that entity and pushes a crumb</strong> onto a sticky breadcrumb trail (Base ▸ Table ▸ Field); the back arrow or a crumb pops; drilling a different branch truncates the trail past the branch point. Descriptions use <strong>two source tabs — Airtable (Public) and Internal</strong>, each with a role meta line. The Airtable copy is <strong>read-only</strong> in Baseout: it shows the description captured at the last backup, and Baseout never writes back to Airtable — schema write-back moves to the future <a href="/agents">Agents</a> section. Only the Internal copy is editable, and it is Baseout-only (never synced to Airtable). <strong>AI is not a stored tab</strong> but a <a href="#button">Generate</a> action (Pro+, cost shown on the button) on the <strong>Internal</strong> tab, prompted to be more technical/verbose. Edits autosave on navigate so nothing is lost. Sections: Context+type, Descriptions, Children, Relationships, Documentation (the docs that tag it). Built from <a href="#drawer">Drawer</a> visuals + <a href="#badge">Badge</a> + <a href="#status-dot">status dots</a> + <a href="#button">Button</a>; the body is rendered at runtime so its styles are global. Component: <code>components/schema/EntityPanel.astro</code>.',
    ...
    usageDo: [
      ...
      'Keep the Airtable description read-only — show the captured copy + its scope meta line; Baseout never writes back to Airtable (write-back moves to the Agents section).',
      'Keep the Internal tab Baseout-only (an "Internal · never synced" meta line) — it never syncs to Airtable, but it CAN be AI-generated via its own Generate action (prompted to be more technical/verbose than the public copy). AI stays a Generate action on the Internal tab, not a third stored description.',
