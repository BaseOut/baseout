...
Managed R2 paused per [`system-r2-park`](../../openspec/changes/system-r2-park/proposal.md); dev + prod write paths are BYOS via the `StorageWriter` interface (see [`server-byos-destinations`](../../openspec/changes/server-byos-destinations/proposal.md)). The Trigger.dev backup-base task continues to write CSVs to `apps/server/.backups/` in dev via `trigger/tasks/_lib/local-fs-write.ts` until the first BYOS strategy (Google Drive) lands.
