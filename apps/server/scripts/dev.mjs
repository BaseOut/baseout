#!/usr/bin/env node
// Local dev launcher for apps/server.
//
// Direct postgres-js TLS from workerd hangs under `wrangler dev`, so the master
// DB must go through wrangler's local Hyperdrive simulator. Wrangler takes that
// connection string from CLOUDFLARE_HYPERDRIVE_LOCAL_CONNECTION_STRING_<BINDING>
// in process.env — it does NOT read .dev.vars into process.env itself. This
// script is invoked as `node --env-file-if-exists=.dev.vars scripts/dev.mjs`, so
// Node loads .dev.vars into process.env and the spawned wrangler inherits it.
// That keeps the real credential out of the committed wrangler.jsonc, whose
// localConnectionString is a deliberate dummy.
//
// Replaces the old scripts/launch.mjs, which rendered wrangler.jsonc from a
// .example template purely to inject this one value.
