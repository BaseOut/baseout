...
//        invalid         → status='pending_reauth' (cron never writes
//                          terminal 'invalid' — only apps/web retires rows)
...
export async function runOAuthRefreshTick(
  ...
  for (const row of candidates) {
    ...
    try {
      ...
      // Cipher tampered with or wrong key. Surface Reconnect without writing
      // terminal 'invalid' — apps/web owns hard invalidation (duplicate retire).
      await deps.db.execute(sql`
        UPDATE baseout.connections
