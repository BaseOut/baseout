// POST /api/internal/spaces/:spaceId/automations/mutate
//
// Manual create / update / soft-remove for bo_at_automations.
//   { action: 'create', baseId, airtableEntityId?, name?, type?, definition?, tags? }
//   { action: 'update', id, name?, type?, definition?, tags? }
//   { action: 'remove', id }
//
...
import { mutateAutomation } from "../../../../lib/per-space/automations-interfaces-io";
...
export async function spacesAutomationsMutateHandler(
  request: Request,
  _env: Env,
  ...
  try {
    ...
    const result = await withSpaceSchema(masterDb, space.pgLocator, (tx) =>
      mutateAutomation(tx, body as Parameters<typeof mutateAutomation>[1]),
    );
    if (!result.ok) {
      return jsonResponse({ error: result.code }, statusFor(result.code));
    }
    if ("entity" in result) {
      return jsonResponse({ ok: true, automation: result.entity }, 200);
    }
