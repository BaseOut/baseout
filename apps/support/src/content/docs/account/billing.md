...
**How many bases a Space may include.** Set at the Space's scope. When a Space is at its limit,
adding a base is what tells you. See [Schedule and scope](/backups/schedule-and-scope/).

**How many Spaces the organization may have.** A Space is bound to one platform, so this is also the
answer to "can I add a second platform". See
[How Baseout is organized](/start/how-baseout-is-organized/).

**Runs.** An off-schedule backup consumes credits beyond the scheduled ones, which is why **Run
...
**Storage**, where you use a Baseout-managed store. Point a Space at your own Google Drive, Dropbox,
...
going to work. Nothing in the product is hidden behind a plan you are not on: it is visible, and it
says what it needs.
