...
**Backups** — one row per run, for every run the Space has ever attempted. Each row carries the
status, the kind, when it finished, what triggered it, and its base, record and attachment counts
and duration.

**A run** — status, trigger, start and finish, duration, totals across the run, which layers it
captured, where it wrote, and a table of every base with its own counts and output location.

**A base within a run** — the leaf. Every table in that base with its Fields, Records, Views, and
Attachments, plus the folder or database it wrote to.
...
| **Running** | In progress. |
| **Paused** | Started, then paused by someone. Can be resumed. |
| **Succeeded** | Finished. |
| **Failed** | Did not finish. Carries a reason. |
| **Cancelled** | Stopped by a person. Keeps whatever it had already written. |
| **Trial run** | A pre-payment run, taken during onboarding. |
...
:::note
**Succeeded does not mean every single thing landed.** A run can finish successfully and still have
skipped individual attachments. That is reported separately, below, rather than by downgrading the
whole run.
:::
...
A dash means the value is **genuinely absent**, not zero and not hidden. A queued run has no record
count because nothing has been counted yet.
