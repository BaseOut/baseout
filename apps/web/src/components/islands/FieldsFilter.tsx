/**
 * FieldsFilter — hierarchical field-visibility filter (Base ▸ Table ▸ Field) for
 * the Visualize canvas (web-schema-visualize; ported from apps/design).
 *
 * Presentational + callback-driven: takes the schema (bases/tables) + the current
 * hidden-field set and emits the next set; the consumer (SchemaCanvas) owns what
 * "hidden" does (it drops the field row). The selection MATH is the shared
 * lib/schema-docs/field-visibility model (groupState / visibleCount /
 * setFieldsVisible) — this component holds no duplicate tri-state logic. The
 * model speaks in a VISIBLE set; this component's external contract is a HIDDEN
 * set (what the canvas stores), so calls invert at the boundary:
 * `setFieldsVisible(hidden, ids, hide)` treats the hidden set as the target set
 * (add on hide, remove on show) — same pure operation, inverted semantics.
 *
 * Tri-state per group: a field is on/off; a table/base is checked when all its
 * fields are visible, unchecked when none, indeterminate when partial. Search
 * filters the tree (keeping ancestors); bulk actions apply to the matches.
 */
...
import {
  AIRTABLE_FIELD_ICONS,
  airtableIconKey,
  fieldTypeLabel,
} from '../../lib/schema-docs/airtable-field-icons'
...
export default function FieldsFilter({ bases, tables, hidden, onChange }: FieldsFilterProps) {
      ...
      .map((b) => ({
        ...
        tables: tables
          ...
          .map((t) => ({
            id: t.id,
            name: t.name,
            fields: t.fields.filter((f) => !f.removed).map((f) => ({ id: f.id, name: f.name, type: f.type })),
          }))
  ...
  // The shared model speaks in a VISIBLE set — derive it from the hidden set.
  const visible = useMemo(() => new Set(allIds.filter((id) => !hidden.has(id))), [allIds, hidden])
    ...
    return tree.flatMap((b) => {
      ...
      const tbs = b.tables.flatMap((t) => {
        ...
        const fields = t.fields.filter(
          (f) => f.name.toLowerCase().includes(ql) || fieldTypeLabel(f.type).toLowerCase().includes(ql),
        )
  ...
  const stateOf = (ids: string[]): TriState => groupState(ids, visible)
  const count = (ids: string[]) => ({ vis: visibleCount(ids, visible), total: ids.length })
  // Boundary inversion (see header): the hidden set is the mutated set — add on hide.
  const apply = (ids: string[], hide: boolean) => onChange(setFieldsVisible(hidden, ids, hide))
  // Tri-state click: checked → hide all; unchecked/indeterminate → show all.
  const toggleGroup = (ids: string[]) => apply(ids, stateOf(ids) === 'checked')
  ...
  const setAllCollapsed = (c: boolean) => {
    ...
    tree.forEach((b) => {
      all.add(b.id)
      b.tables.forEach((t) => all.add(t.id))
    })
    setCollapsed(all)
  }
    ...
    <div className="dropdown">
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '8px 1px 8px' }}>
          <button type="button" className="btn btn-xs btn-ghost gap-1 px-2" disabled={globalState === 'checked'} onClick={() => apply(scopeIds, false)} style={globalState !== 'checked' ? { color: 'var(--color-primary)' } : undefined}>
            <span aria-hidden style={{ width: 13, height: 13, display: 'grid', placeItems: 'center' }}><Eye /></span>Show all{bulkLabel}
          </button>
          <button type="button" className="btn btn-xs btn-ghost gap-1 px-2" disabled={globalState === 'unchecked'} onClick={() => apply(scopeIds, true)}>
            <span aria-hidden style={{ width: 13, height: 13, display: 'grid', placeItems: 'center' }}><EyeOff /></span>Hide all{bulkLabel}
          </button>
          {!ql && (
            <button type="button" className="btn btn-xs btn-ghost px-1.5" style={{ marginLeft: 'auto', opacity: 0.7 }} onClick={() => setAllCollapsed(collapsed.size === 0)} aria-label={collapsed.size === 0 ? 'Collapse all groups' : 'Expand all groups'}>
                    ...
                    <div key={t.id}>
                      ...
                      {topen && t.fields.map((f) => (
                        <Row
                          key={f.id}
                          indent={2}
                          state={hidden.has(f.id) ? 'unchecked' : 'checked'}
                          onToggle={() => apply([f.id], !hidden.has(f.id))}
