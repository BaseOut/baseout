...
export function wireTableSort(
  ...
  /**
   * Called after a HEADER CLICK reorders the rows — a paged tab must re-take its window, since a
   * new order means page 1 is a different set of rows. Not fired by `reapply()`, so a caller can
   * safely re-sort from inside its own apply() without recursing.
   */
  onSorted?: () => void,
) {
  let col = -1;
