...
export function pagerHtml(state: {
  ...
  return `
    <div class="tpg${className ? ` ${className}` : ''}">
      ...
      <div class="ml-auto text-sm text-base-content/65"><span class="mono-data" data-pager-range>${range}</span> of <span class="mono-data" data-pager-total>${total.toLocaleString()}</span></div>
      <div class="tpg-btns">
