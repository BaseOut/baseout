/**
 * Encrypted HttpOnly cookie carrying Box OAuth handoff state.
 *
 * Mirror of [../google-drive/cookie.ts]. The cookie travels from POST
 * /api/connections/storage/box/authorize (generator) through Box's consent
 * redirect to GET /callback (consumer). The JSON payload is encrypted with
 * BASEOUT_ENCRYPTION_KEY (AES-256-GCM); the GCM auth tag doubles as tamper
 * detection so no separate HMAC is needed.
 */

import { decryptToken, encryptToken } from '../crypto'

export const BOX_OAUTH_COOKIE = 'bo_oauth_box'
export const BOX_OAUTH_COOKIE_PATH = '/api/connections/storage/box'
...
export function buildSetCookie(
  ...
  const parts = [
    `${BOX_OAUTH_COOKIE}=${value}`,
    `Path=${BOX_OAUTH_COOKIE_PATH}`,
    'HttpOnly',
    'SameSite=Lax',
  ]
...
export function readHandoffCookie(cookieHeader: string | null): string | null {
  ...
  const match = cookieHeader
    .split(';')
    .map((c) => c.trim())
    .find((c) => c.startsWith(`${BOX_OAUTH_COOKIE}=`))
  if (!match) return null
  const value = match.slice(BOX_OAUTH_COOKIE.length + 1)
