...
describe('getRedirectUri', () => {
  ...
  it('returns env.DROPBOX_REDIRECT_URI verbatim when set, ignoring origin', () => {
    ...
    // browser is on localhost. Pinning the redirect URI to a registered
    // hostname unblocks the OAuth flow without requiring boss to add a new
    // URI to the Dropbox App Console.
    const env = {
      DROPBOX_REDIRECT_URI:
        'https://localhost:4331/api/connections/storage/dropbox/callback',
    }
    expect(
      getRedirectUri('https://baseout-dev.openside.workers.dev', env),
    ).toBe('https://localhost:4331/api/connections/storage/dropbox/callback')
  })
