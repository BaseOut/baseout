...
describe('buildSetCookie', () => {
  it('uses the Dropbox-scoped path so the cookie is sent only to the dropbox routes', () => {
    const header = buildSetCookie('abc', { secure: true, maxAgeSeconds: 600 })
    expect(header).toContain(`${DROPBOX_OAUTH_COOKIE}=abc`)
    expect(header).toContain(`Path=${DROPBOX_OAUTH_COOKIE_PATH}`)
