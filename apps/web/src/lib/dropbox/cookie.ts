/**
 * Encrypted HttpOnly cookie carrying Dropbox OAuth handoff state.
 *
 * Mirror of [../box/cookie.ts] and [../google-drive/cookie.ts]. The cookie
 * travels from POST /api/connections/storage/dropbox/authorize (generator)
 * through Dropbox's consent redirect to GET /callback (consumer). The JSON
...
export function readHandoffCookie(cookieHeader: string | null): string | null {
  ...
  const match = cookieHeader
    .split(';')
    .map((c) => c.trim())
    .find((c) => c.startsWith(`${DROPBOX_OAUTH_COOKIE}=`))
  if (!match) return null
  const value = match.slice(DROPBOX_OAUTH_COOKIE.length + 1)
