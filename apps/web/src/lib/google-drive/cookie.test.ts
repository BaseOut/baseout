...
describe('buildSetCookie', () => {
  it('sets HttpOnly, SameSite=Lax, scoped path, Secure by default', () => {
    ...
    expect(header).toContain(`${GOOGLE_DRIVE_OAUTH_COOKIE}=abc`)
    expect(header).toContain('HttpOnly')
    expect(header).toContain('SameSite=Lax')
    expect(header).toContain('Path=/api/connections/storage/google-drive')
...
describe('readHandoffCookie', () => {
  it('extracts the cookie value', () => {
    const header = `foo=bar; ${GOOGLE_DRIVE_OAUTH_COOKIE}=myvalue; baz=qux`
