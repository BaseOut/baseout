/**
 * Encrypted HttpOnly cookie carrying OneDrive OAuth handoff state.
 *
 * Mirror of [../dropbox/cookie.ts] and [../box/cookie.ts]. The cookie
 * travels from POST /api/connections/storage/onedrive/authorize (generator)
 * through Microsoft's consent redirect to GET /callback (consumer). The JSON
 * payload is encrypted with BASEOUT_ENCRYPTION_KEY (AES-256-GCM); the GCM
 * auth tag doubles as tamper detection so no separate HMAC is needed.
 */

import { decryptToken, encryptToken } from '../crypto'

export const ONEDRIVE_OAUTH_COOKIE = 'bo_oauth_onedrive'
export const ONEDRIVE_OAUTH_COOKIE_PATH = '/api/connections/storage/onedrive'
...
export function buildSetCookie(
  ...
  const parts = [
    `${ONEDRIVE_OAUTH_COOKIE}=${value}`,
    `Path=${ONEDRIVE_OAUTH_COOKIE_PATH}`,
    'HttpOnly',
    'SameSite=Lax',
  ]
...
export function readHandoffCookie(cookieHeader: string | null): string | null {
  ...
  const match = cookieHeader
    .split(';')
    .map((c) => c.trim())
    .find((c) => c.startsWith(`${ONEDRIVE_OAUTH_COOKIE}=`))
  if (!match) return null
  const value = match.slice(ONEDRIVE_OAUTH_COOKIE.length + 1)
