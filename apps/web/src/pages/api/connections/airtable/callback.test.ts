/**
 * Pins the post-OAuth redirect origin for the Airtable callback (parity with
 * OneDrive's 871f4d5 fix). Under `wrangler dev --remote` — or any stale
 ...
 * Airtable differs from the storage providers: result param is `error` and
 * the default returnTo is `/`.
 */
...
const WRONG_HOST_CALLBACK =
  'https://baseout-dev.openside.workers.dev/api/connections/airtable/callback?error=access_denied'

describe('Airtable callback — post-OAuth redirect origin', () => {
  beforeEach(() => {
  ...
  it('redirects to an absolute URL on PUBLIC_AUTH_BASE_URL when set', async () => {
    ...
    expect(res.headers.get('Location')).toBe(
      'https://baseout.local:4331/?error=access_denied',
    )
  })
