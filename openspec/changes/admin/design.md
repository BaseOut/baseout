...
`admin` is the internal operator surface. Astro SSR pages served by a standalone Cloudflare Workers project at `apps/admin/`, deployed independently of every other Baseout repo. It uses the same component library (`@baseout/ui`) as `web` for visual consistency, and consumes the same master-DB schema (`@baseout/db-schema`), but it has its own auth (Google Workspace SSO) and its own deploy cadence. It does NOT share runtime, sessions, or secrets with `web`.

Stakeholders: Baseout staff (operators, on-call), `server` team (downstream of force-backup / force-migration / invalidate-connection actions), legal/compliance (audit trail correctness, retention).
...
**Non-Goals:**
- Customer-facing UI of any kind (`web`).
...
- Stripe webhook handling (`web`).
- Direct execution of long-running workflows (admin invokes backend endpoints; the workflow runs in `server`).

## Decisions

### Standalone Cloudflare Workers project (vs. routes inside `server`)
Earlier consolidation considered hosting admin as Astro routes inside `server` with hostname-based routing. The current decision splits admin out into its own Workers project because:
...
Audit log uses a dedicated Postgres role with INSERT-only privileges (no UPDATE, no DELETE). Application code cannot mutate audit rows — even with a code bug or malicious deploy. Older rows archived to R2 after 24 months via a periodic job (could live in `server`'s background services, or a small admin-side worker entry — TBD pre-launch).

### Manual admin actions invoke `server` internal endpoints
Admin doesn't run long workflows. Force-backup writes the `backup_runs` row and POSTs to `server`'s `/runs/{id}/start`, same path `web` uses (different caller, same contract). Force-migration triggers the migration script via an admin-only endpoint in `server`. Invalidate-connection writes the `connections.status='invalid'` directly (audit row first).

### Reuse `@baseout/ui` component library
Admin uses the same Table, Modal, Form, Tooltip primitives as `web`. Visual consistency, faster build, single styling system.
...
- **[Trade-off] Separate Workers project = small duplication** → Master-DB connection setup and a few helpers duplicated vs. living inside `server`. Accepted; the deploy independence is worth it.
...
|---|---|---|
| A1 | Audit log archive owner | V1: implement archive job in `admin` itself (dedicated cron entry); revisit if it gets complex. |
