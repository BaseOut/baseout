...
- [ ] 1.6 Populate Cloudflare Secrets (master DB string, master encryption key, Google SSO client secret, service-to-`server` HMAC)
...
- [ ] 4.3 Build "Force backup" action (writes `backup_runs`; calls `server`'s `/runs/{id}/start`)
...
- [ ] 5.2 Implement 24-month retention + R2 archive job (cron entry within `admin` worker)
...
## 7. Definition of Done — `admin` V1 Launch
