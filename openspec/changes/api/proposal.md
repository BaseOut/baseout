...
The Inbound API (`/api/v1/inbound/*`) is a public, token-authorized programmatic API used by Airtable Scripts, AI agents, and external automations to submit data Airtable's REST API does not expose (Automations, Interfaces, Synced Tables, custom metadata). It needs its own URL-versioned contract that NEVER breaks v1, its own deploy and version cadence (independent of UI iteration in `web` or data-plane changes in `server`), and a clear ownership boundary as the public auth/validation/rate-limit/billing layer for back-side ingestion. None of it exists yet. This change moves the Inbound API out of `web` (where it was originally proposed) into its own runtime repo `inbound-api` and converts the spec for that repo.

## What Changes

- Establish `inbound-api` as a standalone Cloudflare Workers project at `apps/inbound-api/`, deployed independently of every other Baseout repo.
- Public hostname (e.g., `api.baseout.com`) with URL-versioned paths under `/v1/inbound/*`.
- Bearer-token authentication using the shared `api_tokens` table (master DB), same hash + plaintext-once-at-creation model `web` uses for token CRUD.
...
- Forward validated payloads to `server`'s internal `/inbound/{type}` endpoints (HMAC service-token-authed); `inbound-api` SHALL NOT write to client DBs directly.
...
- `inbound-api`: Public versioned ingestion API at `api.baseout.com/v1/inbound/*` with Bearer-token auth (`api_tokens`), Zod validation per endpoint, tier-based monthly rate limits, per-call credit consumption, forwarding to `server`'s internal ingestion endpoints, and OpenAPI documentation at `docs.baseout.com`.

### Modified Capabilities

None — this is the initial `inbound-api` implementation.
...
- **Cross-repo contracts**:
  - With `web`: shares the `api_tokens` table that `web` manages CRUD on; reads tokens for auth verification; never invokes `web`.
  - With `server`: forwards validated Inbound API payloads to `server`'s internal `/inbound/{type}` endpoints with HMAC service token; receives a write-confirmation response.
...
- **Secrets**: master DB connection string, master encryption key (`api_tokens` hashing), service-to-`server` HMAC.
