...
### Requirement: Forward to `server` ingestion

The Inbound API SHALL NOT write to client DBs directly. It SHALL forward validated payloads to `server`'s `POST /inbound/{type}` ingestion endpoint with the service token; back owns the client DB write.

#### Scenario: Successful forward

- **WHEN** a valid `automations` payload passes validation
- **THEN** `web` POSTs to `server`'s `/inbound/automations` with the validated payload + space_id + service token, and returns the back response to the caller
