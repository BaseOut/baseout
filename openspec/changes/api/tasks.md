...
- [ ] 2.3 Lock token format + auth header contract with `web` (which manages token CRUD)
...
- [ ] 4.1 Implement HMAC service-token forwarder to `server`'s `/inbound/automations`
...
- [ ] 4.5 Lock forwarder contract with `server` before launch
...
## 7. Definition of Done — `inbound-api` V1 Launch

- [ ] 7.1 All 4 endpoints (`/automations`, `/interfaces`, `/synced-tables`, `/custom-metadata`) accept valid tokens, validate payloads, rate-limit, debit credits, and forward to `server`
...
- [ ] 7.7 Cross-repo contracts locked: `api_tokens` shape with `web`, `/inbound/{type}` forwarder shape with `server`
