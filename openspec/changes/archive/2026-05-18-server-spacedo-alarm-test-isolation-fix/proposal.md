# Proposal — server-spacedo-alarm-test-isolation-fix

## Why

Commit `6d887a6` (chore: batch-ship Phase B + theme + openspec proposals) landed Phase B of [server-schedule-and-cancel](../server-schedule-and-cancel/) with **3 known-red tests** in [apps/server/tests/integration/space-do.test.ts](../../../apps/server/tests/integration/space-do.test.ts):
...
All three fail with `expected null to be <some-timestamp>`. They block the Phase B verification gate [`B.7.1`](../server-schedule-and-cancel/tasks.md) (`pnpm --filter @baseout/server test — all green`) and any future CI gate on this branch — and they keep [server-schedule-and-cancel](../server-schedule-and-cancel/) from being archived.
...
- Unblocks Phase B archive for [server-schedule-and-cancel](../server-schedule-and-cancel/) (`B.7.1` flips green).
