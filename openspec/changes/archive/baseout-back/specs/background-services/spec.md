...
Background services SHALL run as cron-triggered handlers within the consolidated `baseout-server` Cloudflare Workers project. The single `wrangler.toml` SHALL declare one cron trigger per service, each routed to its handler under `src/cron/`. Each cron SHALL run idempotently, write stateful flow results to `notification_log`, and be safe to re-run on overlap.
...
#### Scenario: All cron triggers in one wrangler.toml

- **WHEN** an operator inspects the deployed Worker's cron schedule
- **THEN** all background-service crons are listed in the single `baseout-server` `wrangler.toml`, not split across multiple Worker projects
