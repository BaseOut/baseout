...
For runs where `backup_runs.is_trial=true`, the engine SHALL stop the run when any cap is hit — 1,000 records total across all tables, 5 tables max, or 100 attachments max — set `backup_runs.status='trial_complete'`, set `subscription_items.trial_backup_run_used=true`, and trigger the back-owned Trial Cap Hit email. Subsequent backup runs SHALL be blocked at the run-start gate until the trial converts.
...
The engine SHALL persist a per-run audit (counts, errors, skipped) to master DB `backup_runs` and to a client DB audit-log table, emit per-entity verification rows in `backup_run_bases`, and fire the appropriate back-owned email/in-app notification on failure or warning.
