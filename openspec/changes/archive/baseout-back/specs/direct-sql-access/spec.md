...
The connection string SHALL be available via a back read endpoint that decrypts on-demand, never logs the plaintext value, and is accessed only by an authenticated front session for the Org's owner/admin role.

#### Scenario: Owner views connection string

- **WHEN** an Org owner opens the Direct SQL panel and front requests the read endpoint
