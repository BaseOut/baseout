...
- **WHEN** `baseout-web` requests `GET /spaces/{id}/schema/changelog?since=<ts>`
...
If a diff is material (configurable; default: any added/removed field, any type change), the engine SHALL emit an in-app notification and send the `baseout-backup`-owned Schema Change Notification email.
...
- **THEN** the engine writes the new score, fires an in-app notification, and sends the `baseout-backup`-owned Health Score Change email
