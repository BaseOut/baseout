...
The Integrations page SHALL render a Direct SQL section (Business+) that shows the read-only connection string. The string SHALL be fetched via back's read endpoint on user-initiated reveal — never logged.

#### Scenario: Business user reveals string

- **WHEN** a Business user clicks "Show connection string"
- **THEN** front calls back's read endpoint, displays the string in the response, and no log line includes the plaintext
