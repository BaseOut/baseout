...
Front SHALL render a "Complete Your Migration" screen on first login when `organizations.has_migrated=false`. The user SHALL NOT be able to access the dashboard until completion.
...
On successful completion of all re-auth steps, the migration screen SHALL set `organizations.has_migrated=true` and redirect to the dashboard. The Migration Welcome email SHALL fire (per the front email notifications spec).
