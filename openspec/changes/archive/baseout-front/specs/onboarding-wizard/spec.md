...
Step 5 SHALL present a summary panel and a "Run first backup" button. On click, front SHALL write a `backup_runs` row with `trigger_type='manual'` and POST to back's `/runs/{id}/start` endpoint.

#### Scenario: First backup triggered

- **WHEN** a user clicks "Run first backup"
- **THEN** a `backup_runs` row is created and the call to back's `/runs/{id}/start` returns 200
