...
On user sign-up, front SHALL create a Stripe Customer linked to the new Organization (`stripe_customer_id`) and a Subscription with one Trial subscription item per platform (`Baseout — Airtable — Trial`) at $0. No credit card SHALL be required at sign-up.
...
- **THEN** `organization_billing_settings` is updated and back enforces the cap on subsequent runs
...
- **THEN** the receiver triggers the front-owned Trial Expiry Warning email (or back's, per ownership decisions)
