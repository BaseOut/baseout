...
A Trial run SHALL stop at 1,000 records, 5 tables, or 100 attachments — whichever cap is hit first. Cap enforcement SHALL be performed at run time by the back's backup engine.
...
The Day-5 / Day-7 trial expiry detection cron SHALL live in `baseout-background-services` (per the back specs). Front SHALL NOT run trial-expiry detection but SHALL accept the back-generated state and surface it on the dashboard.

#### Scenario: Day-7 conversion

- **WHEN** back converts a trial to paid on day 7 via Stripe
- **THEN** the front capability resolver returns the new tier on next refresh
