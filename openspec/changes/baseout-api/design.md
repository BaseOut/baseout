...
`baseout-inbound-api` is the public versioned ingestion API. It's a thin Cloudflare Workers project whose only job is to accept token-authenticated HTTP POSTs from external scripts and AI agents, validate the payloads, enforce tier-based rate limits, debit credits, and forward the validated payload to `baseout-backup`'s internal ingestion endpoints. It does not own auth state (`api_tokens` is in master DB, managed by `baseout-web`), does not own client DB writes (`baseout-backup` does), and does not render UI (`baseout-web` provides token CRUD).

Stakeholders: external developers and AI agents (the API consumers), `baseout-web` team (token CRUD owner), `baseout-backup` team (downstream of forwarded payloads), product (versioning + rate-limit policy), revenue (credit consumption rate).
...
**Goals:**
- A request from an external script reaches the receiver, gets authenticated, validated, rate-limit-checked, credit-debited, and forwarded in <300ms median (excluding `baseout-backup` write time).
...
**Non-Goals:**
- Token CRUD UI (`baseout-web`).
- Client DB writes (`baseout-backup`).
- Email notifications.
- Capability resolution beyond the per-token tier check (the rich resolver is in `baseout-web`).
- Anything stateful beyond rate-limit counters in master DB.

## Decisions

### Standalone repo (vs. living in `baseout-web`)
The Inbound API is the public versioned ingestion contract. Versioning is the dominant constraint; `/v1/` MUST never break for external consumers regardless of UI iteration cadence. Splitting it out lets the public API ship on its own deploy schedule and keeps the contract change surface area visible. Trade: duplicated auth logic vs. `baseout-web` (token-hash check) — accepted because the duplication is small (a single `api_tokens` lookup) and the deploy-independence win is large.

### Forward to `baseout-backup` (vs. write to client DBs directly)
This repo authenticates, validates, rate-limits, and debits credits, then POSTs to `baseout-backup`'s internal `/inbound/{type}` endpoints. `baseout-backup` owns the actual client DB write. Reason: client DB ownership stays with the data plane; this repo stays thin and stateless beyond the forward-and-debit transaction. Trade: extra hop (sub-millisecond on Cloudflare service binding); accepted.
...
- **[Trade-off] Duplicated auth logic with `baseout-web`** → A few dozen lines of `api_tokens` hash lookup duplicated. Accepted — much smaller than the deploy-independence win.
...
4. **Phase 3 — Forwarding**: HMAC service-token forwarder to `baseout-backup`'s `/inbound/{type}` endpoints; lock the contract with `baseout-backup` before launch.
