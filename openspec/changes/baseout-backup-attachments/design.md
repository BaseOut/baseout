...
`apps/server/trigger/tasks/_lib/attachment-downloader.ts`:
...
The existing `postProgress` callback (from `baseout-backup-history-live-status` and Phase 10d) gains an `attachmentsDownloaded: number` field. Wire it in `apps/server/trigger/tasks/backup-base.task.ts` after each per-table page completes.
