...
The `baseout-backup` engine emits `[N attachments]` as a placeholder string in every cell where an Airtable attachment field has values. See [apps/server/trigger/tasks/_lib/field-normalizer.ts](../../../apps/server/trigger/tasks/_lib/field-normalizer.ts). No actual file bytes are downloaded; no R2 object is written. The CSV ends up with a count where the data should be.
...
The active change `baseout-backup-schedule-and-cancel` explicitly defers attachments to this change by name in its Out-of-Scope table.
...
- **New module** `apps/server/trigger/tasks/_lib/attachment-downloader.ts`. Pure-ish; injectable HTTP client + R2 + dedup-table accessor.
- **Entry points**:
...
- **Wire into** [apps/server/trigger/tasks/_lib/field-normalizer.ts](../../../apps/server/trigger/tasks/_lib/field-normalizer.ts) — replace the `[N attachments]` placeholder branch with a call to `downloadAndStoreAttachment` per attachment.
...
This phase is small but optional. If it adds review burden, defer to a follow-up `baseout-backup-attachments-opt-out`.

### Phase F — Doc sync

- Update [openspec/changes/baseout-backup-schedule-and-cancel/proposal.md](../baseout-backup-schedule-and-cancel/proposal.md) "Out of Scope" table — link this change as the resolved follow-up.
- Update [openspec/changes/baseout-backup/proposal.md](../baseout-backup/proposal.md) Out-of-Scope section if it still references attachments as a placeholder.

## Out of Scope

| Deferred to | Item |
|---|---|
| Future change `baseout-backup-attachment-restore` | Restore-from-snapshot path for attachments. Today's restore engine doesn't exist; when it does, it'll need to map the composite ID back to an Airtable upload. Separate concern. |
| Future change `baseout-backup-attachment-dedup-by-content` | Dedup by `content_hash` instead of composite ID. Today: composite-ID dedup means the same file with different `attachment_id` is downloaded twice. Future optimization. |
...
| Bundled with `baseout-backup-byos-destinations` | Proxy streaming for Box/Dropbox. The dedup table + downloader land here; the destination-strategy wrapper lands there. |
| Bundled with `baseout-backup-dynamic-mode` | Storing attachment metadata in a per-Space client DB (D1 / Postgres). Today: dedup table is in master DB across all Spaces. Dynamic-mode change relocates it. |
| Bundled with `baseout-backup-trial-quota-enforcement` | Trial-cap enforcement for the 100-attachment limit. Engine emits the count; quota enforcement is upstream of the per-attachment download. |
...
- **Phase D** (trial cap): blocking-dependent on `baseout-backup-trial-quota-enforcement`; reversal removes the cap check.
