...
- [ ] F.2 Update `apps/server/trigger/tasks/backup-base.task.ts` (or a new per-run finalization step):
  - Static mode: write `<runId>/submitted-<type>.json` via `writer.writeFile`.
  - Dynamic mode: UPSERT into `_baseout_<type>` tables.
- [ ] F.3 Tests for both static + dynamic inclusion.
...
- [ ] OUT-1 `baseout-submitted-entities-validation` — JSON-schema validation per entity type.
- [ ] OUT-2 `baseout-submitted-entities-restore` — Restore back into Airtable.
- [ ] OUT-3 `baseout-submitted-entities-diff` — Diff a new version against previous.
- [ ] OUT-4 `baseout-submitted-entities-ai-import` — LLM-assisted bulk import.
- [ ] OUT-5 `baseout-submitted-entities-retention` — Prune old versions beyond N per logical entity.
- [ ] OUT-6 `baseout-submitted-entities-reminders` — Email reminder for tier-eligible Spaces with no submissions.
