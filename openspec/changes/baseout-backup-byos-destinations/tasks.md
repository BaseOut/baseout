...
- [ ] B.2.3 Refactor `backup-base.task.ts` to call `makeStorageWriter` instead of direct `env.BACKUPS_R2.put`. Existing tests should still pass.
...
### E.1 — Strategy selection in backup-base.task.ts

- [ ] E.1.1 Update `apps/server/trigger/tasks/backup-base.task.ts` to call `loadStorageDestination(spaceId)` + `makeStorageWriter(dest, env, masterKey)` + `writer.init()` + `writer.writeFile(...)`.
- [ ] E.1.2 Branch on `writer.proxyStreamMode` per design.md §Phase F.
- [ ] E.1.3 Wire attachment-downloader output to the same writer (`writer.writeFile` is called from the downloader).

### E.2 — Tests

- [ ] E.2.1 Extend `apps/server/tests/integration/backup-base-task.test.ts` with one scenario per provider (mocked). Seeded `storage_destinations` row; assert the correct strategy is called.
...
- [ ] G.1 Update [openspec/changes/baseout-backup/specs/storage-destinations/spec.md](../baseout-backup/specs/storage-destinations/spec.md) — link this change as the implementation.
- [ ] G.2 Update [shared/Baseout_Backlog_MVP.md](../../../shared/Baseout_Backlog_MVP.md).
- [ ] G.3 Update [openspec/changes/baseout-backup-schedule-and-cancel/proposal.md](../baseout-backup-schedule-and-cancel/proposal.md) Out-of-Scope table.
