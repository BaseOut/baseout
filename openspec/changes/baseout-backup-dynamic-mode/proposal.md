...
- **Trigger.dev task** `apps/server/trigger/tasks/provision-space-database.task.ts` invokes the appropriate function per tier. Set on Stripe webhook upgrade or first-backup trigger.
...
- Update [openspec/changes/baseout-backup/specs/backup-engine/spec.md](../baseout-backup/specs/backup-engine/spec.md) — dynamic mode requirement now has an implementation change.
- Update [openspec/changes/baseout-backup-schedule-and-cancel/proposal.md](../baseout-backup-schedule-and-cancel/proposal.md) — link as resolved follow-up.
