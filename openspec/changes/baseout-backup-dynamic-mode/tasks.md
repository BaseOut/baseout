...
### B.6 — Trigger.dev task

- [ ] B.6.1 `apps/server/trigger/tasks/provision-space-database.task.ts` invokes the dispatcher with retry.
- [ ] B.6.2 Registered in `trigger.config.ts`.
...
### C.2 — Wire into backup-base.task.ts

- [ ] C.2.1 Branch on `config.mode === 'dynamic'` per design.md §Phase C. Static path continues to run; dynamic adds the second write.
- [ ] C.2.2 Block on `space_databases.status !== 'ready'` — if not ready, log `event: 'dynamic_db_not_ready_skip'` and continue with static-only for this run.

### C.3 — Tests

- [ ] C.3.1 `apps/server/tests/integration/backup-base-task.dynamic.test.ts` — seeded `space_databases` row + run; assert dynamic DB has expected schema + records.
...
- [ ] D.3 Wire into `backup-base.task.ts` — at end of each table's schema upsert, compute diff vs previous run + INSERT to `audit_history`.
...
- [ ] H.1 Update [openspec/changes/baseout-backup/specs/backup-engine/spec.md](../baseout-backup/specs/backup-engine/spec.md).
- [ ] H.2 Update [openspec/changes/baseout-backup-schedule-and-cancel/proposal.md](../baseout-backup-schedule-and-cancel/proposal.md) Out-of-Scope.
