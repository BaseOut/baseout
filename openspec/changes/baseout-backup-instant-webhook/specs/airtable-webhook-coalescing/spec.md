...
The per-Space DurableObject SHALL support BOTH a cron alarm (scheduled snapshots, set by `baseout-backup-schedule-and-cancel`) AND a webhook-debounce alarm (this change) using a single underlying DO alarm. State for both fire times SHALL persist in DO storage; the alarm SHALL fire at the minimum of the two.
