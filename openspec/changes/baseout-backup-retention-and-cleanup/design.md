...
- Trigger.dev v3 scheduled tasks already exist in [apps/server/trigger/tasks/](../../../apps/server/trigger/tasks/). Cron coverage, retries, observability come free.
...
`apps/server/trigger/tasks/cleanup-expired-snapshots.task.ts`:
...
- Decides whether the user has credits available (depends on `baseout-backup-manual-quota-and-credits`).
- Calls engine route via `BackupEngineClient.runCleanup(spaceId)`.
- Returns 200 / 402 / 404.

### Credit charge interlock

If `baseout-backup-manual-quota-and-credits` has shipped:
...
- The `baseout-backup-schedule-and-cancel` SpaceDO alarm. Cleanup runs on its own cron, independent of the per-Space backup schedule.
