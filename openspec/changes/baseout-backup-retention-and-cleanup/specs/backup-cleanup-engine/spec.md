...
A Trigger.dev v3 scheduled task `cleanup-expired-snapshots` SHALL run on an hourly cron (`0 * * * *`) and execute a single full cleanup pass across every Space. The task SHALL be defined in `apps/server/trigger/tasks/cleanup-expired-snapshots.task.ts` and registered in `apps/server/trigger.config.ts`.
