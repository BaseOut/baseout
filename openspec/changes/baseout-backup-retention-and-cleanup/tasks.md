...
### C.3 — Trigger.dev scheduled task

- [ ] C.3.1 New file `apps/server/trigger/tasks/cleanup-expired-snapshots.task.ts` per design.md §Phase C.2 (cron task). Calls `runCleanupPass`.
- [ ] C.3.2 Add the export to `apps/server/trigger.config.ts` `tasks` array.
- [ ] C.3.3 Smoke locally: `pnpm --filter @baseout/server trigger:dev` (or whatever the existing dev command is). Manually invoke via the Trigger.dev dashboard test runner. Assert the task completes and writes the expected `event: 'backup_cleanup_pass'` log.
...
- [ ] D.2.4 Credit interlock: gate the engine call on `chargeCredits(orgId, 10, 'cleanup_manual_trigger')` per [Features §5.2](../../../shared/Baseout_Features.md). If `baseout-backup-manual-quota-and-credits` hasn't shipped, log a stub `would_charge_10_credits` event + proceed; feature-flag the UI button off in prod.
...
- [ ] F.3 Update [openspec/changes/baseout-backup-schedule-and-cancel/proposal.md](../baseout-backup-schedule-and-cancel/proposal.md) "Out of Scope" table — add a row linking this change as the resolved follow-up.
...
- [ ] OUT-1 `baseout-backup-retention-custom-editor` — Rich GUI editor for Business/Enterprise Custom policy. First-pass MVP renders a JSON textarea with schema validation.
- [ ] OUT-2 `baseout-backup-cold-storage-tier` — Lifecycle rules that move snapshots older than X to a cheaper R2 / S3 storage class instead of deleting outright.
- [ ] OUT-3 `baseout-backup-restore-from-soft-deleted` — 7-day grace window before R2 objects are unrecoverable. Today: R2 delete is immediate.
- [ ] OUT-4 `baseout-backup-byos-cleanup` — Cleanup of BYOS destinations. Today: BYOS-destination retention is the customer's responsibility per Features §6.6.
- [ ] OUT-5 `baseout-audit-history-retention` — Schema-changelog retention per Features §7.2. Different table (`audit_history`), different cleanup loop.
