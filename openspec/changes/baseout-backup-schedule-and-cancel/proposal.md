...
- Update [openspec/changes/baseout-backup/proposal.md](../baseout-backup/proposal.md) Out-of-Scope section to reference the same follow-up changes by name so the chain is discoverable from either entry point.
...
| Deferred to | Item |
|---|---|
| `baseout-backup-attachments` (new openspec change) | Real attachment download + R2 upload. PRD §6.3 lists attachments as Must-Have for all tiers; the engine currently emits a `[N attachments]` placeholder per [field-normalizer.ts](../../../apps/server/trigger/tasks/_lib/field-normalizer.ts). Genuine MVP gap, separate concern. |
