...
- [x] A.2.5 Verify the existing `backup-base.task.ts` outer try/catch handles `AbortError` thrown by Trigger.dev's cancellation injection. Add an `apps/server/tests/integration/backup-base-task-cancel.test.ts` that simulates the abort and asserts `runBackupBase`'s finally block fires the ConnectionDO unlock.
...
- [ ] C.2 Update [openspec/changes/baseout-backup/proposal.md](../baseout-backup/proposal.md) Out-of-Scope section to reference the same follow-ups by name.
...
- [ ] C.4 Update [openspec/changes/baseout-backup-schedule-and-cancel/proposal.md](./proposal.md) Out-of-Scope section: as new follow-up changes get filed, link to them from here so future readers can traverse the dependency graph.
