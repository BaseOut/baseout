...
### E.3 — Day 5 / Day 7 cron

- [ ] E.3.1 New task `apps/server/trigger/tasks/trial-email-cron.task.ts` per design.md §Phase E.
- [ ] E.3.2 Daily cron schedule via `schedules.task`.
- [ ] E.3.3 Idempotency: `trial_expiry_warning_sent_at` + `trial_expired_email_sent_at` columns prevent duplicates.
- [ ] E.3.4 Tests: pure function `decideTrialEmails(row, now)` → which emails to send.
...
- [ ] H.1 Update [openspec/changes/baseout-backup/specs/backup-engine/spec.md](../baseout-backup/specs/backup-engine/spec.md) trial-cap requirement — link this change as the pre-flight gate.
