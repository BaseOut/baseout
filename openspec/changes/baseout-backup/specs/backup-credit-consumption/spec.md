...
The `baseout-backup` repo SHALL report period-close metered usage to Stripe directly via the Stripe API (no `baseout-web` coordination), so cross-repo period boundaries do not require synchronization.

#### Scenario: Period close

- **WHEN** a billing period closes for an Org
- **THEN** `baseout-backup` reads the period's overage transactions, calls Stripe's `customer.subscription.report_usage`, and marks the period reported
