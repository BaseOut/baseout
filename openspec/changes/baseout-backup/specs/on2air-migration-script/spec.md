...
The migration script SHALL live in `baseout-backup` as a one-shot entry point under `src/migration/` (or equivalent) — manually invoked, not on a cron schedule, and not exposed as a public route.
