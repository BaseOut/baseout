...
- **THEN** the `baseout-web`-side validation rejects it; if it slips past, the engine refuses to use it and marks the run failed with reason `tier_not_eligible_destination`
