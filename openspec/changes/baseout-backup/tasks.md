...
- [ ] 1.1 Create `apps/backup/` repo with README, Vitest, Drizzle, msw, Wrangler config, directory layout (`src/handlers/`, `src/cron/`, `src/durable-objects/`, `src/trigger-dev/`, `src/emails/`, `src/migration/`, `src/lib/`)
- [ ] 1.2 Wire GitHub Actions CI on `apps/backup/` (Vitest on every PR; Docker PG + Miniflare D1 in CI)
...
- [ ] 3.7 Implement Restore Complete email (`baseout-backup`-owned)
...
- [ ] 6.8 Implement Stripe metered usage reporting cron (period-close, `baseout-backup`-direct)
...
- [ ] 7.2 Wire Mailgun SDK with `baseout-backup`-scoped API key + sending domain `mail.baseout.com`
...
## 8. Definition of Done — `baseout-backup` V1 Launch
...
- [ ] 8.16 All `baseout-backup`-owned React Email templates send via Mailgun
