...
- **WHEN** `baseout-backup` or `baseout-backup` imports table definitions
...
`baseout-web` SHALL publish the schema as an internal npm package `@baseout/db-schema` consumed by `baseout-backup` at a pinned version.

#### Scenario: Version pinned by back

- **WHEN** `baseout-backup`'s `baseout-backup` updates Drizzle types
- **THEN** the back repo pins a specific `@baseout/db-schema` version and re-publishes via PR / lockfile change

### Requirement: Connection management

The master DB connection string SHALL live in Cloudflare Secrets. `baseout-web` SHALL connect via Drizzle for all reads/writes; `baseout-backup` SHALL use its own Drizzle client and its own connection string from its Cloudflare Secrets.
