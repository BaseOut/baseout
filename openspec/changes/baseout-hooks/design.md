...
`baseout-webhook-ingestion` is the public-facing Cloudflare Worker that Airtable POSTs webhook callbacks to. It's deliberately thin: verify HMAC, look up the row, forward to `baseout-backup`'s per-Space DO. No coalescing, no persistence, no run-triggering — those concerns live in `baseout-backup`'s `airtable-webhook-coalescing` capability so that the public contract (this repo) and the data-plane consumption (`baseout-backup`) can be versioned and deployed independently.

Stakeholders: Airtable (the upstream caller), `baseout-backup` team (downstream consumer of forwarded events), security/on-call (signature verification correctness, rejection-rate monitoring).

Constraints carried in from product:
- **Independent versioning + deploy** — auth or routing changes should ship without redeploying any other repo. Conversely, `baseout-backup` deploys must not interrupt webhook reception.
...
- **No client DB writes** — those are `baseout-backup`'s job.
...
- Independent deploy: `baseout-backup` redeploys do not require this repo to redeploy and vice versa.

**Non-Goals:**
- Event coalescing (`baseout-backup`).
- Cursor advancement (`baseout-backup`).
- Gap detection / full re-read fallback (`baseout-backup`).
- Webhook lifecycle (registration on Instant Backup enable, renewal cron — `baseout-backup`).
...
### Service binding to `baseout-backup` (preferred over public HTTP)
...
Dedup happens in `baseout-backup`'s per-Space DO (so duplicate forwards are a no-op there). Doing dedup here would require state, which we want to avoid.
...
- **[Risk] `baseout-backup` deploy in progress causes forward failures** → Receiver returns 503; Airtable retries per its policy; brief delivery delay but no data loss.
...
- **[Trade-off] No event-id dedup at receiver** → Pushes dedup responsibility to `baseout-backup`. Accepted because dedup needs state and this repo is stateless beyond DB reads.
...
3. **Phase 2 — Forward**: service binding to `baseout-backup`'s per-Space DO; lock the forward contract with `baseout-backup` before launch.
