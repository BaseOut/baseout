...
- [ ] 3.1 Implement service binding to `baseout-backup`'s per-Space DO (preferred path)
...
- [ ] 3.6 Lock forward contract with `baseout-backup` before launch
...
- [ ] 5.3 Test downstream-failure handling: simulate `baseout-backup` outage; verify Airtable retries are accepted on recovery
- [ ] 5.4 Optionally add Cloudflare KV caching for `airtable_webhooks` lookups if load test shows DB latency dominates

## 6. Definition of Done — `baseout-webhook-ingestion` V1 Launch

- [ ] 6.1 Receiver verifies HMAC and rejects invalid signatures cleanly
- [ ] 6.2 Receiver looks up Space and forwards verified events to `baseout-backup`
- [ ] 6.3 No event reaches `baseout-backup` without HMAC verification
- [ ] 6.4 Forward failures return 5xx so Airtable retries
- [ ] 6.5 Receiver continues accepting callbacks during a `baseout-backup` deploy (with brief 5xx-and-retry behavior, no event loss)
- [ ] 6.6 Observability + alerting wired
- [ ] 6.7 Independent deploy verified — receiver redeploys without `baseout-backup` redeploy and vice versa
