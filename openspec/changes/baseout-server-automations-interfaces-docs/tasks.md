...
- [ ] F.2 Workflows-side finalization (writes `<runId>/submitted-<type>.json` in static mode, UPSERTs `_baseout_<type>` tables in dynamic mode) lives in [`baseout-workflows-automations-interfaces-docs`](../baseout-workflows-automations-interfaces-docs/tasks.md).
