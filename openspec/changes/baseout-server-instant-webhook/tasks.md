...
- [ ] D.1 Incremental-backup task itself owned by [`baseout-workflows-instant-webhook`](../baseout-workflows-instant-webhook/tasks.md).
...
- [ ] G.1 Update [openspec/changes/baseout-server/specs/airtable-webhook-coalescing/spec.md](../baseout-server/specs/airtable-webhook-coalescing/spec.md) — link this change.
- [ ] G.2 Update [openspec/changes/baseout-server-schedule-and-cancel/proposal.md](../baseout-server-schedule-and-cancel/proposal.md) Out-of-Scope.
...
- [ ] OUT-1 `baseout-backup-instant-conflict-resolution` — Handling cases where webhook payload disagrees with dynamic-DB state.
- [ ] OUT-2 `baseout-backup-instant-snapshot-rollup` — Periodic consolidation snapshots after N incrementals.
- [ ] OUT-3 `baseout-backup-instant-multi-base-coalesce` — Cross-base coalesce optimization.
- [ ] OUT-4 `baseout-backup-webhook-replay` — Replay-from-cursor recovery.
- [ ] OUT-5 `baseout-backup-instant-cursor-monitoring` — Alert when cursor falls behind (real-time backup latency).
