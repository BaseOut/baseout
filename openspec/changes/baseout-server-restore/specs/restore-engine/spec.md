...
Restore progress and completion payloads SHALL share field names + types with the backup-side `/api/internal/runs/:runId/{progress,complete}` payloads (substituting `runId → restoreId`, `runStartedAt → restoreStartedAt`, etc.). This makes WebSocket broadcasts trivially extend to restore frames per [`baseout-web-websocket-progress`](../../baseout-web-websocket-progress/specs/web-live-progress/spec.md).
