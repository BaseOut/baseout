# baseout-server-workspace-rediscovery
...
The schema scaffolding shipped in commit `3eeedfb` (Phase 1 of this change). The pure orchestrator + dep wiring + manual rescan route are in-flight on `autumn/server-setup` (Phase 2 of this change). Alarm-driven scheduled rediscovery is **deferred to Phase 3** until `baseout-server-schedule-and-cancel` archives — the alarm code path is the wrong place to introduce two-writer concerns simultaneously.
...
See [proposal.md](./proposal.md), [design.md](./design.md), [tasks.md](./tasks.md), and the paired UI change [`baseout-web-workspace-rediscovery`](../baseout-web-workspace-rediscovery/proposal.md).
