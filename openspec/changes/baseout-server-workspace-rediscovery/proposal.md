...
Out of scope for this change. Lands after [`baseout-server-schedule-and-cancel`](../baseout-server-schedule-and-cancel/proposal.md) archives — that change owns the SpaceDO alarm scheduler. Phase 3 will:
...
| Paired [`baseout-web-workspace-rediscovery`](../baseout-web-workspace-rediscovery/proposal.md) | Frontend Rescan button, banner UI, `space_events` dismiss endpoint, auto-add toggle in Integrations view. |
| [`baseout-server-instant-webhook`](../baseout-server-instant-webhook/proposal.md) | Webhook-driven real-time drift detection inside already-selected bases (not a rediscovery concern). |
| [`baseout-server-dynamic-mode`](../baseout-server-dynamic-mode/proposal.md) | Schema-diff inside individual bases (handled by the per-run backup task today). |
