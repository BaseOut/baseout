...
Out of scope. Tracked separately. Will pick up after [`baseout-server-schedule-and-cancel`](../baseout-server-schedule-and-cancel/proposal.md) archives.
...
- Smoke (covered in the paired [`baseout-web-workspace-rediscovery`](../baseout-web-workspace-rediscovery/proposal.md)): click Rescan → banner appears with correct counts → dismiss removes banner → server log shows rediscovery counts.
