...
`baseout-server` is the data plane: long-running workflows, Durable Objects, cron handlers, Trigger.dev jobs, client DB writes, and the operational email categories that fire from any of the above. It's deployed as one Cloudflare Workers project at `apps/server/`, independent of `baseout-web`, `baseout-inbound-api`, `baseout-sql-rest-api`, `baseout-webhook-ingestion`, and `baseout-admin`. It does NOT serve a public Airtable webhook endpoint (that's `baseout-webhook-ingestion`), a public token-authed Inbound API (that's `baseout-inbound-api`), a public SQL endpoint (that's `baseout-sql-rest-api`), or any customer-facing UI (that's `baseout-web` + `baseout-admin`).

Stakeholders: engineering (the `baseout-server` team owning the data plane), operations (super-admin users in `baseout-admin` consuming `baseout-server`'s state + on-call), `baseout-web` team (consumers of run-trigger contracts + WebSocket progress + read endpoints), `baseout-webhook-ingestion` team (forwarders of verified Airtable events), `baseout-inbound-api` team (forwarders of validated Inbound payloads), customers (especially Pro+ for Instant Backup; Business+ for Direct SQL).
...
- `baseout-server` deploys independently — backup engine bug fixes don't require redeploying any other repo, and conversely.
...
The public `POST /webhooks/airtable/{webhook_id}` endpoint lives in `baseout-webhook-ingestion`, NOT here. `baseout-server` only owns the per-Space DO that *consumes* verified forwards. Reason: independent versioning of the public Airtable contract (signature scheme rotations, hostname/route changes) without redeploying the data plane; conversely backup-engine deploys do not interrupt webhook reception. Trade: an extra service-binding hop per event; mitigated by Cloudflare's intra-account binding latency being sub-millisecond.

### Inbound API receiver split into its own repo
Same pattern: `baseout-inbound-api` owns the public token-authed `/api/v1/inbound/*` endpoint and forwards validated payloads to `baseout-server`'s internal `/inbound/{type}` endpoints. `baseout-server` writes to client DBs.

### Stripe metered usage reporting from `baseout-server`, not via `baseout-web`
`baseout-server` already holds Stripe credentials for read-only use (subscription state) and is the side that produces the overage transactions. Reporting metered usage from `baseout-server` avoids a period-close coordination dance with `baseout-web`. Resolved (B4).
...
All `baseout-server`-owned React Email templates live in one place — `apps/server/src/emails/` — and are imported by whichever handler (HTTP, cron, Trigger.dev workflow callback) detects the trigger. No duplication across repos.
...
- **[Risk] Webhook forward failure between `baseout-webhook-ingestion` and `baseout-server`** → If the per-Space DO is unreachable during a `baseout-server` deploy, the receiver returns 503 so Airtable retries; brief delivery delay but no data loss.
...
### Build sequence (mirrors the original `Back_Implementation_Plan.md` phases, scoped to `baseout-server`):
...
7. **Phase 6 — Email Templates + Pre-Launch Hardening**: All `baseout-server`-owned React Email templates, Logpush + tail Workers, on-call routing, error-rate alerts, load + stress test, security review.
...
| B4 | Stripe metered usage reporting | **Resolved**: `baseout-server`-direct. |
