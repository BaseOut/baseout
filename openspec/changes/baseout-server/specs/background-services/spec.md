...
Background services SHALL run as cron-triggered handlers within the `baseout-server` Cloudflare Workers project. The single `wrangler.jsonc` SHALL declare one cron trigger per service, each routed to its handler under `src/cron/`. Each cron SHALL run idempotently, write stateful flow results to `notification_log`, and be safe to re-run on overlap.
...
- **THEN** all background-service crons are listed in the single `baseout-server` `wrangler.jsonc`, not split across multiple Worker projects
