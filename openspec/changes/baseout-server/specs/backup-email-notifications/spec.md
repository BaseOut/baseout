...
Sends SHALL use sending domain `mail.baseout.com` (DKIM/SPF/DMARC). `baseout-server`'s Mailgun API key SHALL be a separate key from `baseout-web`'s, held in Cloudflare Secrets.
