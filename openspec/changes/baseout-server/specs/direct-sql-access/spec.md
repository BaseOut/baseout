...
The connection string SHALL be available via a `baseout-server` read endpoint that decrypts on-demand, never logs the plaintext value, and is accessed only by an authenticated front session for the Org's owner/admin role.
