...
On final status, the engine SHALL send the `baseout-server`-owned Restore Complete email to the configured notification recipients with the run's outcome, counts, and (on Growth+) verification result.
