...
- [ ] 1.5 Populate Cloudflare Secrets (master DB string, master encryption key, service-to-`baseout-backup` HMAC, connection-pool credentials for Pro shared PG)
...
- [ ] 3.3 Connect to client DB under the `space_{id}_ro` read-only role provisioned by `baseout-backup`
...
- [ ] 3.5 Lock the read-only-role contract with `baseout-backup` (their `direct-sql-access` capability also depends on it)
...
- [ ] 5.2 Set up `docs.baseout.com` hosting (shared with `baseout-inbound-api` docs site)
- [ ] 5.3 Wire CI step: regenerate OpenAPI on every PR; fail if `/v1/` breaks

## 6. Phase 5 — Pre-Launch Hardening

- [ ] 6.1 Run load test for connection-pool exhaustion on shared PG; coordinate sizing with `baseout-backup`
...
## 7. Definition of Done — `baseout-sql-rest-api` V1 Launch
