...
| Dev resolution | `wrangler dev --remote` for apps/web → deployed `baseout-server-dev` | Multi-worker local dev (both with `wrangler dev`, no `--remote`) is a fallback path, out of scope |
...
apps/server already declares `env.staging` (Worker `baseout-server-staging`) and `env.production` (Worker `baseout-server`). Add a parallel `env.dev` block (Worker `baseout-server-dev`) so apps/web's dev service binding has a deploy target:
...
### Primary path: deploy `baseout-server-dev` once, iterate via redeploy
...
2. **Daily dev:**
   ...
   ```
   Open `https://localhost:4331/integrations`, click **Test connection** → request flows through the service binding to the deployed `baseout-server-dev`.
...
1. Deploy `apps/server` to staging: `pnpm --filter @baseout/server deploy:staging`. This creates Worker `baseout-server-staging`.
