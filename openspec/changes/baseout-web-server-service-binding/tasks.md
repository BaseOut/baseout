...
- [x] 3.2 ~~Add `services` blocks to `env.staging` and `env.production`.~~ **Deferred to follow-up openspec change** [`baseout-web-server-service-binding-staging-prod`](../baseout-web-server-service-binding-staging-prod/) per agreement: those Workers (`baseout-server-staging`, `baseout-server`) aren't deployed yet; declaring the binding now would fail-resolve at deploy. The follow-up change covers staging+prod wiring, secret setup, and the legacy `BACKUP_ENGINE_URL` cleanup.
