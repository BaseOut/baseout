...
- [ ] 2.1 Edit [apps/server/wrangler.jsonc](../../../apps/server/wrangler.jsonc): add an `env.dev` block above `env.staging`. Worker name `baseout-server-dev`. Reuse the existing dev Hyperdrive `id` (`ba2652f40f864918a2da0849f24d12a2`); reuse `localConnectionString: "{{DATABASE_URL}}"` so [apps/server/scripts/launch.mjs](../../../apps/server/scripts/launch.mjs) (or its equivalent) can render it. No new bindings beyond Hyperdrive — DOs and migrations are top-level and inherit per CLAUDE.md §5.1 conventions (verify the inheritance is in place; if not, redeclare).
- [ ] 2.2 Add a `deploy:dev` script to [apps/server/package.json](../../../apps/server/package.json): `"deploy:dev": "wrangler deploy --env dev"` mirroring the existing `deploy:staging`/`deploy:production` shape.
