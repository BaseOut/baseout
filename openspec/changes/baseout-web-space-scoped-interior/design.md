...
| Idempotency guard | `data-sidebar-wired` sentinel on the sidebar root, **only if double-binding is observed** | Mirrors `data-theme-wired` from [baseout-web-smooth-theme-swap](../baseout-web-smooth-theme-swap/design.md). Defer until smoke verifies. |
