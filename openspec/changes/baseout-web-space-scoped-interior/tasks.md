...
- [ ] 4.2 **Only if double-fire is observed**: add a `data-sidebar-wired` sentinel — set on the sidebar root inside `init()`; early-return if already set. Mirror of the `data-theme-wired` pattern from [baseout-web-smooth-theme-swap](../baseout-web-smooth-theme-swap/tasks.md). Skip this step if smoke is clean.
...
- [ ] 8.4 Archive this change via `/opsx:archive baseout-web-space-scoped-interior` once committed.
