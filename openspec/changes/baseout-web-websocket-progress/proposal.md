...
This change implements the WebSocket leg of the broader `web-client-isolation` principle: browser opens `wss://baseout.com/api/ws/spaces/{id}/progress`, `apps/web`'s route validates the session + Space ownership, then forwards the WebSocket upgrade to `apps/server`'s `SpaceDO` via a cross-Worker Durable Object namespace binding. The browser never sees `apps/server`'s URL.
...
- `web-client-boundary` (from `web-client-isolation`): the WebSocket-side of the isolation principle is now concrete. After this change ships, the HTTP-side (already shipped via `INTERNAL_TOKEN` service binding) plus the WebSocket-side complete the original `web-client-isolation` proposal.
...
- The HTTP-side proxy routes (`/api/runs/:id/start`, `/api/runs/:id/cancel`, etc.) — already shipped via service binding + `INTERNAL_TOKEN`. The `web-client-isolation` change tracks any remaining HTTP-side cleanup.
...
- Restore-progress events — restore engine doesn't exist yet (`baseout-server-restore` is a separate proposal). When it lands, this WebSocket fan-out extends naturally to `{ type: 'restore_progress', restoreId, ... }`.
