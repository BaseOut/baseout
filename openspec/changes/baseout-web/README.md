...
Customer-facing app at `apps/web/` (`@baseout/web`). Astro SSR + better-auth + Drizzle on Cloudflare Workers. Ported from `baseout-starter` HEAD `29dfb5b` with the abandoned `baseout-backup-engine` RPC wire surgically removed.
