...
`baseout-web` SHALL detect embedded context via a URL parameter AND the check `window.parent !== window`. Only when both signals match SHALL the app render the embedded layout.
...
`baseout-web` SHALL implement a `window.postMessage` handler that exchanges context (current base, table, view) with a thin wrapper running inside the Airtable extension. The messaging contract SHALL be documented and stable.

#### Scenario: Wrapper sends context

- **WHEN** the wrapper posts `{ type: 'context', baseId, tableId, viewId }`
- **THEN** `baseout-web` updates the active selection in the embedded layout
