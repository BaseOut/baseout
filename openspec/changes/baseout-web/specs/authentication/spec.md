...
All user authentication flows SHALL use better-auth. Auth concerns SHALL live exclusively in `baseout-web`; back services SHALL NOT log a user in directly — they validate session tokens or service tokens minted by front.

#### Scenario: Back receives a request

- **WHEN** `baseout-backup`'s `baseout-backup` receives a request from front
- **THEN** the request carries a service token (HMAC) or a session-derived short-lived token; no other repo runs a user-login flow

### Requirement: V1 authentication methods

`baseout-web` SHALL support: Magic Link (V1 launch), Email + Password (pre-launch V1), 2FA TOTP (pre-launch V1), SSO SAML for Enterprise (pre-launch V1). Google OAuth MAY be evaluated pre-launch. Airtable OAuth SHALL NEVER be a login method — it is exclusively a Connection auth flow.
...
Service-to-service traffic SHALL use a shared HMAC `Authorization: Bearer {SERVICE_TOKEN}` header. `baseout-web` SHALL NOT call any other repo. Other repos MAY call `baseout-web` internal API endpoints with the service token; front validates on every internal call.

#### Scenario: Other-repo-to-`baseout-web` internal call

- **WHEN** `baseout-backup` POSTs to a front internal endpoint
- **THEN** `baseout-web` verifies the HMAC service token before processing
