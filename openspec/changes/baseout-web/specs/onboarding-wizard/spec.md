...
Step 5 SHALL present a summary panel and a "Run first backup" button. On click, `baseout-web` SHALL write a `backup_runs` row with `trigger_type='manual'` and POST to `baseout-backup`'s `/runs/{id}/start` endpoint.

#### Scenario: First backup triggered

- **WHEN** a user clicks "Run first backup"
- **THEN** a `backup_runs` row is created and the call to `baseout-backup`'s `/runs/{id}/start` returns 200
