...
The Schema page SHALL render a React Flow node graph of the Space's schema (bases → tables → fields), populated from `baseout-backup`'s `GET /spaces/{id}/schema` read endpoint.
...
The Schema page SHALL include a changelog tab that renders human-readable diffs from `baseout-backup`'s `GET /spaces/{id}/schema/changelog?since=...` endpoint.
...
The Schema page SHALL display the per-Base health score and band from the `baseout-backup`-computed result, plus a drill-in panel for the contributing rule details.
...
The Schema page SHALL include a rule-configuration UI (Pro+) that writes `health_score_rules` for the Org. Configuration SHALL surface the same default rules `baseout-backup` uses if none are configured.
