...
A Trial run SHALL stop at 1,000 records, 5 tables, or 100 attachments — whichever cap is hit first. Cap enforcement SHALL be performed at run time by the `baseout-backup`'s backup engine.
...
The Day-5 / Day-7 trial expiry detection cron SHALL live in `baseout-backup` (per the `baseout-backup` specs). `baseout-web` SHALL NOT run trial-expiry detection but SHALL accept the `baseout-backup`-generated state and surface it on the dashboard.

#### Scenario: Day-7 conversion

- **WHEN** `baseout-backup` converts a trial to paid on day 7 via Stripe
- **THEN** the `baseout-web` repo capability resolver returns the new tier on next refresh
