...
Pre-req: `baseout-server-attachments` Phase A (DB schema) + `baseout-server-byos-destinations` Phase B (storage writer) are green.
...
- [ ] 5.2 Cross-check: server-side `baseout-server-attachments` task list no longer carries workflows file-path bullets; only the schema + route + dedup-storage bullets remain.
