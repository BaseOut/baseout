...
Every in-flight openspec change that adds or modifies a Trigger.dev task SHALL exist as a `baseout-workflows-<topic>` change paired with the corresponding `baseout-server-<topic>` change. The workflows-side change SHALL own the task definition, helpers, and tests; the server-side change SHALL own the enqueue path, master-DB writes, DO state, and callback handlers.

#### Scenario: paired changes
- **WHEN** an in-flight openspec change includes file paths under `apps/workflows/trigger/`
- **THEN** a sibling change with name `baseout-workflows-<topic>` SHALL exist
- **AND** the sibling SHALL cross-reference the `baseout-server-<topic>` change in its `proposal.md`
