...
This change captures the workflows app boundary itself. Per-task work (the actual task definitions for backup-base, cleanup, attachments, etc.) lives in the sibling `baseout-workflows-<topic>` changes.
...
- [x] 5.1 `git mv openspec/changes/baseout-backup → openspec/changes/baseout-server`
- [x] 5.2 `git mv openspec/changes/baseout-server-<topic> → openspec/changes/baseout-server-<topic>` for every in-flight change in the family
- [x] 5.3 Update `apps/server/openspec` symlink target to `../../openspec/changes/baseout-server`
- [x] 5.4 Create `apps/workflows/openspec` symlink → `../../openspec/changes/baseout-workflows`
- [ ] 5.5 For each in-flight `baseout-server-<topic>` that touches Trigger.dev tasks, create a sibling `baseout-workflows-<topic>` change with proposal.md + tasks.md scoping the workflows-side work; trim those bullets from the server-side change.
