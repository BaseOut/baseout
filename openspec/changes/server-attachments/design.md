...
- **Decision for MVP**: trust the 8min-or-less typical case. If a task fails on attachment count, the next scheduled run will hit ~all-dedup and complete quickly. The `server-attachment-checkpointing` follow-up adds explicit resumability if this becomes an operational problem.
...
Depends on `server-trial-quota-enforcement`. Two integration points:
...
| Trial | covered by `server-trial-quota-enforcement` |
