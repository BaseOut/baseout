> **Depends on**: [`system-r2-park`](../system-r2-park/proposal.md). The destination-write path lives in the paired [`workflows-attachments`](../workflows-attachments/tasks.md) change and now targets the BYOS `StorageWriter` from [`server-byos-destinations`](../server-byos-destinations/proposal.md) Phase B (interface) + Phase C (per-provider strategies). Managed R2 is paused per `system-r2-park`; "write the attachment to R2" is parked pending R2 revival. The tasks in **this** file (server-side: dedup-schema in Phase A; composite-id pure helpers in B.1; lookup-endpoint in B.2; observability/trial/UI in C–F) are storage-strategy-independent and can ship before any provider strategy lands. The end-to-end smoke in Phase G needs both this change and `workflows-attachments` (which needs at least one BYOS strategy — Google Drive ships first per `server-byos-destinations`).
...
The downloader implementation, field-normalizer integration, and per-task plumbing are owned by [`workflows-attachments`](../workflows-attachments/tasks.md). The server side owns the dedup-lookup engine-callback endpoint that the workflows downloader hits.
...
`ATTACHMENTS_DRY_RUN` env var on the Trigger.dev runner; downloader logic in `workflows-attachments` honors it.

### B.5 — Backup-base task test extension (moved)

Per-task integration test (`backup-base-task.test.ts` with attachments fixture) lives under `apps/workflows/tests/` per the workspace split. Tracked in `workflows-attachments` Phase 4.
...
- [ ] C.2.1 Extend the `/runs/complete` route handler to accept `attachmentCountByBase: { [baseId]: number }` in the payload. The workflows-side task posts it per `workflows-attachments`.
...
This phase documents the integration points; actual implementation depends on `server-trial-quota-enforcement` shipping.
...
May be deferred to a separate `server-attachments-opt-out` change if review burden is high.
...
- [ ] F.1 Update [openspec/changes/server-schedule-and-cancel/proposal.md](../server-schedule-and-cancel/proposal.md) "Out of Scope" table — link this change as the resolved follow-up.
- [ ] F.2 Update [openspec/changes/server/proposal.md](../server/proposal.md) Out-of-Scope section if it references attachments as a placeholder.
...
- [ ] OUT-1 `server-attachment-checkpointing` — Resumable per-task attachment processing for very large bases. MVP relies on dedup-on-retry; this adds explicit checkpoint state.
- [ ] OUT-2 `server-attachment-restore` — Restore path that maps composite IDs back to Airtable uploads. Depends on the restore engine landing.
- [ ] OUT-3 `server-attachment-dedup-by-content` — Dedup by `content_hash` rather than composite ID; catches the case where the same file is re-uploaded under a new attachment ID.
- [ ] OUT-4 `server-attachments-opt-out` (if Phase E is deferred) — Pro+ opt-out toggle as a standalone change.
- [ ] OUT-5 `server-attachment-cdn-signing` — Customer-facing signed URLs for direct R2 attachment access. Restore-adjacent.
