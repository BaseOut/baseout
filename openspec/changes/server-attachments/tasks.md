...
> **Landed 2026-06-08 (branch `autumn/backup-fix-local`):** Phase A schema (canonical `attachment_dedup` in `apps/web/src/db/schema/core.ts` + migration `0013_attachment_dedup.sql` + engine mirror `apps/server/src/db/schema/attachment-dedup.ts`) and the dedup endpoints (`POST /api/internal/attachments/lookup` + sibling `/record`, routing-layer tests in `apps/server/tests/integration/attachments-lookup-route.test.ts`) shipped together with the workflows downloader (`workflows-attachments`). **Deviations from the plan below:** the `r2_object_key` column is the destination-agnostic `storage_key`; a `/record` upsert route was added alongside `/lookup`; the migration is `0013` (not `0009`); `/runs/complete` already persisted `attachmentsProcessed` so no change there. **Deferred (still `[ ]`):** Phase C observability, Phase D trial cap, Phase E Pro+ opt-out.
