...
The token is the Org's API token (generated separately per `public-api-tokens`).
...
The static-mode JSON sits next to the records CSV in R2. The dynamic-mode upsert lands in `_baseout_<type>` tables (defined in `server-dynamic-mode`).
