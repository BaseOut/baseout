...
The existing openspec change `server/specs/storage-destinations/spec.md` already defines the `StorageWriter` interface contract (`init`, `writeFile`, `getDownloadUrl`, `delete`). This change is its implementation.
...
- The engine's record-page and attachment-download paths already operate on `ReadableStream` (per `server-attachments`). For `proxyStreamMode` writers, the engine SHALL NOT first stage the bytes in R2 — it pipes Airtable's response stream directly through `writer.writeFile`.
...
- The retention engine (`server-cleanup-engine`) calls `writer.delete(path)` per expired snapshot. Each strategy implements the delete semantics for its provider (file delete for OAuth providers, S3 `DeleteObject`).
- The future restore engine (not in scope) will call `writer.getDownloadUrl(path)` to fetch the snapshot for restore.

### Phase G — Doc sync

- Update [openspec/changes/server/specs/storage-destinations/spec.md](../server/specs/storage-destinations/spec.md) — link this change as the implementation.
- Update [shared/Baseout_Backlog_MVP.md](../../../shared/Baseout_Backlog_MVP.md) — tick the BYOS rows.
- Update [openspec/changes/server-schedule-and-cancel/proposal.md](../server-schedule-and-cancel/proposal.md) — link as resolved follow-up.
...
| Future change `server-byos-cleanup` | Retention cleanup of BYOS destinations. Today: BYOS retention is the customer's responsibility; managed-R2 retention is automated (see `server-retention-and-cleanup`). |
| Future change `server-restore-from-byos` | Restore-from-snapshot for BYOS destinations. Depends on restore engine. |
| Bundled with `server-attachments` | Proxy streaming pattern for attachments. The Stream→writer plumbing in this change reuses the attachment downloader's `ReadableStream` shape. |
| Bundled with `server-cron-oauth-refresh` | Token-refresh cron for OAuth providers. Extends the existing refresh path to cover storage tokens. |
