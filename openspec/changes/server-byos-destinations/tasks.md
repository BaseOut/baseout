...
Workflows-side wiring lives in [`workflows-byos-destinations`](../workflows-byos-destinations/tasks.md). Server side owns the `loadStorageDestination(spaceId)` engine-callback (route + DB query); workflows side calls it.
...
- [ ] G.1 Update [openspec/changes/server/specs/storage-destinations/spec.md](../server/specs/storage-destinations/spec.md) — link this change as the implementation.
- [ ] G.2 Update [shared/Baseout_Backlog_MVP.md](../../../shared/Baseout_Backlog_MVP.md).
- [ ] G.3 Update [openspec/changes/server-schedule-and-cancel/proposal.md](../server-schedule-and-cancel/proposal.md) Out-of-Scope table.
...
- [ ] OUT-1 `server-custom-byos` — Pro+ self-hosted destination with HMAC service-token auth.
- [ ] OUT-2 `server-byos-folder-picker` — Rich GUI folder picker for OAuth destinations.
- [ ] OUT-3 `server-storage-failover` — Auto-failover to managed R2 on extended provider downtime.
- [ ] OUT-4 `server-byos-cleanup` — Optional Baseout-side retention enforcement for BYOS destinations.
- [ ] OUT-5 `server-restore-from-byos` — Restore engine support for BYOS source.
- [ ] OUT-6 Extend `server-cron-oauth-refresh` — Refresh `storage_destinations.oauth_*` tokens proactively rather than lazy.
