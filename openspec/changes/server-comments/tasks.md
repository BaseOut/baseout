...
- [ ] 3.1 `space-db-pg.ts`: `readCommentWorkingSet(recordIds)` / `applyCommentBatch`.
...
- [ ] 3.4 Integration tests (`tests/integration/per-space/comments-sync.test.ts`): first capture; edit; delete; incremental batch leaves unvisited records untouched.
