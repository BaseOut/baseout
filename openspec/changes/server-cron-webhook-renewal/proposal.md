...
- **New module** `apps/server/src/lib/airtable-webhook-renewal.ts`: Airtable-specific refresh RPC. Mirrors the shape of `airtable-refresh.ts` from `server-cron-oauth-refresh`.
- **Extend engine-side schema mirror** `apps/server/src/db/schema/airtable-webhooks.ts` to include `expires_at`, `status`, `last_renewed_at`. No new columns in the canonical master DB; the mirror declares what the engine reads/writes (the canonical migration lands as part of `server-instant-webhook` Phase A).
...
None directly; the `airtable-webhook-coalescing` capability (owned by `server` parent) already references "webhook lifecycle (registration on Instant Backup enable, renewal owned by the in-repo background service)" — this change implements the renewal side.
...
- **Webhook registration on Instant Backup enable** — owned by `server-instant-webhook` Phase E.
- **Webhook event coalescing + cursor advancement** — owned by `server-instant-webhook` Phase B-D.
