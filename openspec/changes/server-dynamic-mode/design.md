...
The `audit_history` table itself is defined in `system-db-schema` (already filed). This change consumes it.
...
if (newTier does NOT support dynamic && oldTier did):
  ...
  // Decommission scheduled in dynamic-db-decommission follow-up
```
