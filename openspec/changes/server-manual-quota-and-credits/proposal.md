...
**One cross-change dependency to flag**: `server-retention-and-cleanup` Phase D charges 10 credits per manual cleanup trigger. The credit-charge surface lives here. If this change ships first, the retention change consumes it. If retention ships first, it ships with a TODO that this change resolves.
...
- Update [openspec/changes/server-schedule-and-cancel/proposal.md](../server-schedule-and-cancel/proposal.md) and [openspec/changes/server-retention-and-cleanup/proposal.md](../server-retention-and-cleanup/proposal.md) to link this change as the resolved credit-charge surface.
...
| Deferred to | Item |
|---|---|
| Future change `billing-stripe-overage-reporting` | End-of-period Stripe usage events for overage billing. This change tracks `overage_credits` but does not push to Stripe. |
| Future change `credit-addon-purchase` | Stripe add-on purchase flow for one-time / recurring credit packs per Features §5.4. |
| Future change `credit-promo-grants` | Admin-side surface for issuing promotional credits + onboarding credits. |
| Future change `credit-export-csv` | CSV export of the ledger for accounting. |
| Bundled with `server-trial-quota-enforcement` | Trial-tier credit grant ($0 + activity pauses when credits exhausted per Features §5.1). Trial has no overage path; this change's overage logic treats Trial as `cap, pause, $0_cap`. |
| Bundled with `server-attachments` | Attachment-bytes counting. The engine reports `attachmentBytes` in `/runs/complete`; this change consumes it. If attachments hasn't shipped, `attachmentBytes=0` and the credit cost is just schema + records. |
