...
- [ ] D.1.2 Workflows-side attachment-byte tracking + emission lives in [`workflows-manual-quota-and-credits`](../workflows-manual-quota-and-credits/tasks.md).
...
- [ ] F.1.1 Credit-balance-alerts scheduled task owned by [`workflows-manual-quota-and-credits`](../workflows-manual-quota-and-credits/tasks.md). Server side owns the engine-callback endpoints the task hits (`/api/internal/credit-orgs`, `/api/internal/orgs/:id/credit-alert`) + React Email template render + Cloudflare Workers `send_email` dispatch.
...
- [ ] G.1 Document the contract per design.md §Phase G in this change's README. Phase G implementation lives in a separate `billing-stripe-overage-reporting` change.

## Phase H — Documentation scope-lock

- [ ] H.1 Update [openspec/changes/server-schedule-and-cancel/proposal.md](../server-schedule-and-cancel/proposal.md) — link this change as the resolved credit-charge surface.
- [ ] H.2 Update [openspec/changes/server-retention-and-cleanup/proposal.md](../server-retention-and-cleanup/proposal.md) — link this change as resolving the Phase D credit-charge interlock.
...
- [ ] OUT-1 `billing-stripe-overage-reporting` — End-of-period Stripe usage events for overage billing.
- [ ] OUT-2 `credit-addon-purchase` — Stripe checkout flow for one-time + recurring credit packs.
- [ ] OUT-3 `credit-promo-grants` — Admin surface for issuing promotional / onboarding credits.
- [ ] OUT-4 `credit-export-csv` — Ledger CSV export for accounting.
- [ ] OUT-5 `credit-cost-admin-ui` — Admin UI for editing `credit_cost_config` rates without DB access.
