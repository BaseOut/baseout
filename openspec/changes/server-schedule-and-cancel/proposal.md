...
This change commits to PRD's `Pro+` reading. Instant is out of scope for THIS change anyway (webhook-driven, separate `server-instant` follow-up), but the proposal records the call so the next reader doesn't re-litigate.
...
- Update the master plan at [`~/.claude/plans/use-the-prd-and-eager-stardust.md`](../../../../.claude/plans/use-the-prd-and-eager-stardust.md) "What this plan deliberately does NOT include" table to:
  ...
  - List the deferred follow-ups by name (`server-attachments`, `server-per-table-selection`, `server-instant-webhook`, `server-dynamic-mode`).
- Update [openspec/changes/server/proposal.md](../server/proposal.md) Out-of-Scope section to reference the same follow-up changes by name so the chain is discoverable from either entry point.
...
| `server-pause-resume` (potential follow-up) | Pause / resume a Space's schedule without changing its frequency. MVP achieves the same outcome by setting frequency to a sentinel value or by deleting+recreating the config — neither is great UX. |
