...
The engine has the building blocks today (per [openspec/changes/server/specs/backup-engine/spec.md](../server/specs/backup-engine/spec.md) Requirement "Trial cap enforcement"): it caps records + tables + attachments mid-run and sets `status='trial_complete'`. What's missing:
...
3. **Scheduled-run alarm fire** (in SpaceDO from `server-schedule-and-cancel`): on alarm, check trial state; if `phase === 'active' && runUsed`, skip the alarm-fire INSERT and log `event: 'trial_scheduled_run_skipped_run_used'`.
...
- Update [openspec/changes/server/specs/backup-engine/spec.md](../server/specs/backup-engine/spec.md) trial-cap requirement — point to this change as the implementation of the pre-flight gate.
- Update [openspec/changes/server-schedule-and-cancel/proposal.md](../server-schedule-and-cancel/proposal.md) Out-of-Scope.
...
| Deferred to | Item |
|---|---|
| Future change `trial-restart-on-cancel` | Allow re-trial after the first trial expires. Today: one trial per platform per Org, ever. |
| Future change `trial-extend-by-admin` | Admin-side "grant extra trial days" button. MVP relies on the fixed 7-day window. |
| Future change `trial-multi-platform` | When Notion / Salesforce trials launch, the per-platform trial scoping. MVP: Airtable-only. |
| Future change `trial-checkout-credit-card-precollect` | Pre-collecting card at sign-up (optional, simplifies conversion). MVP: collect at conversion time. |
| Bundled with `server-attachments` | The 100-attachment trial cap. Engine reports `attachmentsDownloaded`; gate consumed here. |
| Bundled with `server-manual-quota-and-credits` | Trial-tier credit cap ($0 → activity pauses). The credit-ledger change treats Trial as `cap, pause, $0`. |
