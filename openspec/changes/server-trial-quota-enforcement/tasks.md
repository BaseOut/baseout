...
- [ ] C.3.1 TDD red: extend the SpaceDO test (from `server-schedule-and-cancel`) with a trial-run-used scenario → alarm fires but skips INSERT + log emitted.
...
Trial-email cron task owned by [`workflows-trial-quota-enforcement`](../workflows-trial-quota-enforcement/tasks.md). Server side owns:
...
- [ ] H.1 Update [openspec/changes/server/specs/backup-engine/spec.md](../server/specs/backup-engine/spec.md) trial-cap requirement — link this change as the pre-flight gate.
...
- [ ] OUT-1 `trial-restart-on-cancel` — Allow re-trial after expiry.
- [ ] OUT-2 `trial-extend-by-admin` — Admin "grant extra days" button.
- [ ] OUT-3 `trial-multi-platform` — Per-platform trial scoping for future Notion/Salesforce.
- [ ] OUT-4 `trial-checkout-credit-card-precollect` — Pre-collect card at sign-up.
- [ ] OUT-5 `trial-conversion-discount` — Auto-apply discount on trial-to-paid conversion within X days.
