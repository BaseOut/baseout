...
The schema scaffolding shipped in commit `3eeedfb` (Phase 1 of this change). The pure orchestrator + dep wiring + manual rescan route are in-flight on `autumn/server-setup` (Phase 2 of this change). Alarm-driven scheduled rediscovery is **split into [`server-rediscovery-alarm`](../server-rediscovery-alarm/proposal.md)**, gated on `server-schedule-and-cancel` archiving — the alarm code path is the wrong place to introduce two-writer concerns simultaneously.
