...
`server` is the data plane: long-running workflows, Durable Objects, cron handlers, Trigger.dev jobs, client DB writes, and the operational email categories that fire from any of the above. It's deployed as one Cloudflare Workers project at `apps/server/`, independent of `web`, `inbound-api`, `sql-rest-api`, `webhook-ingestion`, and `admin`. It does NOT serve a public Airtable webhook endpoint (that's `webhook-ingestion`), a public token-authed Inbound API (that's `inbound-api`), a public SQL endpoint (that's `sql-rest-api`), or any customer-facing UI (that's `web` + `admin`).

Stakeholders: engineering (the `server` team owning the data plane), operations (super-admin users in `admin` consuming `server`'s state + on-call), `web` team (consumers of run-trigger contracts + WebSocket progress + read endpoints), `webhook-ingestion` team (forwarders of verified Airtable events), `inbound-api` team (forwarders of validated Inbound payloads), customers (especially Pro+ for Instant Backup; Business+ for Direct SQL).
...
- **Schema package owned by `packages/db-schema/`** — `server` imports `@baseout/db-schema` and never redefines tables.
- **Email ownership lives with the side that detects the trigger** — `server` dispatches via the Cloudflare Workers `send_email` binding scoped to its own Worker (mirroring `apps/web`'s pattern in `apps/web/src/lib/email/send.ts`).
...
- **The public Airtable webhook receiver is in `webhook-ingestion`** — `server` exposes only an internal forward target via service binding.
...
- Live progress emits structured events on a stable WebSocket path that `web` depends on across the run lifecycle.
...
- `server` deploys independently — backup engine bug fixes don't require redeploying any other repo, and conversely.

**Non-Goals:**
- Public Airtable webhook receiver (that's `webhook-ingestion`).
- Public Inbound API endpoint (`inbound-api`).
- Public SQL REST API endpoint (`sql-rest-api`).
- Customer-facing UI (`web`).
- Admin UI (`admin`).
...
The public `POST /webhooks/airtable/{webhook_id}` endpoint lives in `webhook-ingestion`, NOT here. `server` only owns the per-Space DO that *consumes* verified forwards. Reason: independent versioning of the public Airtable contract (signature scheme rotations, hostname/route changes) without redeploying the data plane; conversely backup-engine deploys do not interrupt webhook reception. Trade: an extra service-binding hop per event; mitigated by Cloudflare's intra-account binding latency being sub-millisecond.

### Inbound API receiver split into its own repo
Same pattern: `inbound-api` owns the public token-authed `/api/v1/inbound/*` endpoint and forwards validated payloads to `server`'s internal `/inbound/{type}` endpoints. `server` writes to client DBs.

### Stripe metered usage reporting from `server`, not via `web`
`server` already holds Stripe credentials for read-only use (subscription state) and is the side that produces the overage transactions. Reporting metered usage from `server` avoids a period-close coordination dance with `web`. Resolved (B4).
...
All `server`-owned React Email templates live in one place — `apps/server/src/emails/` — and are imported by whichever handler (HTTP, cron, Trigger.dev workflow callback) detects the trigger. No duplication across repos.
...
- **[Risk] Webhook forward failure between `webhook-ingestion` and `server`** → If the per-Space DO is unreachable during a `server` deploy, the receiver returns 503 so Airtable retries; brief delivery delay but no data loss.
...
### Build sequence (mirrors the original `Back_Implementation_Plan.md` phases, scoped to `server`):

1. **Phase 0 — Foundation**: `apps/server/` repo + CI/CD, one Cloudflare Workers project per environment with route + cron + DO + R2/D1 bindings, secrets, `@baseout/db-schema` consumed, Trigger.dev account, DigitalOcean shared PG, Neon/Supabase accounts.
2. **Phase 1 — Backup Engine MVP**: Airtable client, per-Connection DO, per-Space DO, static backup → R2, attachment dedup, trial cap, run-trigger endpoint. **Cross-repo checkpoint:** `web` wizard Phase 2 calls `/runs/{id}/start`.
...
5. **Phase 4 — Webhook Coalescing + Inbound Forwarding + Dead-Connection Cadence + On2Air Migration**: per-Space DO event coalescing (forwarded from `webhook-ingestion`), `/inbound/{type}` ingestion endpoints (forwarded from `inbound-api`), dead-connection cadence, trial-expiry monitor, quota monitor, smart-cleanup scheduler, On2Air migration script.
...
7. **Phase 6 — Email Templates + Pre-Launch Hardening**: All `server`-owned React Email templates, Logpush + tail Workers, on-call routing, error-rate alerts, load + stress test, security review.
...
| B4 | Stripe metered usage reporting | **Resolved**: `server`-direct. |
