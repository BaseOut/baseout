...
The backup engine SHALL accept three trigger sources: scheduled (a cron handler within `server` writes a `backup_runs` row and invokes the run-start path internally), manual (`web` writes a `backup_runs` row and POSTs `/runs/{id}/start`), and webhook-driven (an Airtable webhook event is coalesced by the per-Space DO into an incremental run).

#### Scenario: Manual run starts on front trigger

- **WHEN** `web` POSTs `/runs/{run_id}/start` with a valid service token and `backup_runs.status='pending'`
...
- **WHEN** a cron handler within `server` inserts a `backup_runs` row with `trigger_type='scheduled'` and invokes the run-start path
...
For runs where `backup_runs.is_trial=true`, the engine SHALL stop the run when any cap is hit — 1,000 records total across all tables, 5 tables max, or 100 attachments max — set `backup_runs.status='trial_complete'`, set `subscription_items.trial_backup_run_used=true`, and trigger the `server`-owned Trial Cap Hit email. Subsequent backup runs SHALL be blocked at the run-start gate until the trial converts.
...
The engine SHALL persist a per-run audit (counts, errors, skipped) to master DB `backup_runs` and to a client DB audit-log table, emit per-entity verification rows in `backup_run_bases`, and fire the appropriate `server`-owned email/in-app notification on failure or warning.
