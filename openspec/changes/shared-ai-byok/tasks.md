...
- [ ] 1.1 Master-DB migration: `ai_provider_keys` (`organization_id`, `provider` enum, `key_enc`, `key_fingerprint`, `last_four`, `label`, `model_default` nullable, `status` enum `active|invalid|disabled`, `created_by_user_id`, `last_validated_at`, `validation_error` nullable, timestamps) + partial unique index on one **active** key per (org, provider). Generate + apply to dev DB in this change (CLAUDE.md §5.5 — schema-aware SSR 404s on a missing column).
