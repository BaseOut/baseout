...
- [ ] 2.3 Set the four required secrets on `server-staging`:
...
- [ ] 3.1 Repeat §2.1 for `env.production` (use the real prod Hyperdrive ID from §1.2; Worker name `server`).
...
- [ ] 4.1 Edit [apps/web/wrangler.jsonc.example](../../../apps/web/wrangler.jsonc.example) `env.staging`: add a `services` block declaring `BACKUP_ENGINE` → `server-staging`. Mirror the dev shape.
...
- [ ] 5.1 Repeat §4.1 for `env.production` (Worker name `server`).
...
- [ ] 6.3 Archive this change via `/opsx:archive shared-server-service-binding-staging-prod`.

## 7. Roll-back plan (only if a deploy goes sideways)

- [ ] 7.1 If apps/web staging deploy fails because the binding can't resolve: confirm `server-staging` is actually deployed (`wrangler deployments list --env staging` from `apps/server`). Re-run §2.4 if missing.
