...
- **Update [apps/web/.dev.vars.example](../../../apps/web/.dev.vars.example)** — remove `BACKUP_ENGINE_URL`, keep `BACKUP_ENGINE_INTERNAL_TOKEN`. Add a comment block explaining the deploy precondition (`server-dev` must exist).
- **Add a dev env to [apps/server/wrangler.jsonc](../../../apps/server/wrangler.jsonc)** so `apps/web`'s `dev` env service binding has a real Worker to point at: `env.dev` named `server-dev`. This mirrors how `env.staging`/`env.production` already exist on the same file.
...
- **Multi-worker local dev** (running `wrangler dev` on both apps with no `--remote`, with apps/web's binding pointing at a sibling local wrangler-dev instance). This is supported by wrangler v4's `--config` flag, but it's a fallback path — the primary dev flow this change establishes is "deploy `server-dev`, iterate via re-deploy." Multi-worker local dev is a separate change once we know whether the redeploy cost is too high.
...
- **`apps/server` config:** add `env.dev` block to `apps/server/wrangler.jsonc` (Worker name `server-dev`, dev Hyperdrive id, dev secrets). This provides a deploy target that `apps/web`'s dev service binding resolves to.
...
Roll-forward, not roll-back, but the rollback is mechanical: revert the diff to [apps/web/src/lib/backup-engine.ts](../../../apps/web/src/lib/backup-engine.ts) (restore the URL + fetchImpl signature), revert the `services` block in apps/web's wrangler.jsonc, restore `BACKUP_ENGINE_URL` in env.d.ts and .dev.vars.example. The `apps/server` side has no code changes to undo. The `server-dev` deployment can be left in place or torn down via `wrangler delete --name baseout-server-dev` — neither affects the rolled-back path.
