...
The cost: apps/web needs deploy-time visibility into `apps/server`'s DO class name and script name. We pin those in `wrangler.jsonc.example` per env (e.g. `server-dev`, `server-staging`, `server`).
