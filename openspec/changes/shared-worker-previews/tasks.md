...
- [ ] 6.1 Human smoke on the LIVE staging previews (already seeded 2026-08-25 via `wrangler preview --name staging`, no git push needed): open `https://staging.console.baseout.dev` (behind Cloudflare Access — openside.cloudflareaccess.com), magic-link login works (proves preview `BASEOUT_ENCRYPTION_KEY`/`BETTER_AUTH_SECRET`/dev-DB wiring). Server preview healthy (`staging-baseout-server…/api/health` 200); admin preview at `staging-baseout-admin.openside.workers.dev`. NOTE bare console.baseout.dev = PRODUCTION.
- [ ] 6.2 Airtable Connect on the preview stays `status='active'` (proves encryption-key pairing, D4-2) — register callbacks for the CONFIRMED origin `https://staging.console.baseout.dev` first (oauth-setup §4 HOLD lifted).
