...
`sql-rest-api` is the public read-only SQL endpoint. A thin Cloudflare Workers project that authenticates per-Space tokens, parses each query for read-only safety, executes against the Space's client DB under a read-only role, debits credits, and returns rows. It does not own DB provisioning (`server`), token CRUD (`web`), or any UI.

Stakeholders: external developers (BI/dashboard tools, scripts), `server` team (provisions read-only roles, owns client DB lifecycle), product (versioning + rate-limit policy), revenue.
...
- **Structurally symmetric to `inbound-api`** — same `api_tokens` table, same OpenAPI publishing pattern, similar credit-consumption semantics. Sharing this design language across the two public APIs reduces customer learning curve.
...
- Rate limit + credit accounting matches `inbound-api`'s patterns for consistency.

**Non-Goals:**
- Write operations of any kind.
- Direct PG connection-string surfacing (that's `server`'s `direct-sql-access` capability for Business+).
- Token CRUD (`web`).
- Read-only role provisioning (`server`).
...
Public versioned API; `/v1/` MUST never break. Structural twin of `inbound-api`. Splitting it out lets it ship on its own deploy schedule. Trade: shares connection-pool sizing concerns with `server` (both connect to the same Pro shared PG instance) — coordinate via shared infrastructure docs and pool config.

### Two-layer read-only enforcement
Layer 1: `server` provisions a `space_{id}_ro` role with SELECT-only privileges on the Space's client DB. The SQL REST API connects under this role.
...
Bucketed counter — 50 queries debits 1 credit. Reason: cheap-by-design to encourage iteration; aligns with `inbound-api`'s 1-per-100-calls pattern.

### Connection pool shared with `server` for shared PG
For Pro tier (shared PG), both `sql-rest-api` and `server` connect to the same DigitalOcean PG cluster. Pool sizing must be coordinated. For Dedicated PG (Business+), each Space has its own DB; this repo connects directly per request via the per-Space connection string from `space_databases` (decrypted on-demand).

## Risks / Trade-offs

- **[Risk] Connection pool exhaustion on shared PG** → Coordinate sizing with `server`; cap concurrent SQL REST connections per Org; load test before launch.
...
- **[Risk] Token leak** → Same logging policy as `inbound-api`: never log plaintext bearer.
- **[Trade-off] Shared `api_tokens` table with `inbound-api`** → A token created for Inbound API is also valid for SQL REST. Could be confusing. V1 accepts this — both consume the same shared scope. V2 may add per-API token scope flags if confusion shows up.
...
3. **Phase 2 — Read-Only Enforcement**: query parser (rejects non-SELECT), read-only role connection (depends on `server` Phase 5 read-only role provisioning), parameterization requirement.
