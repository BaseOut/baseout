...
**Non-Goals:**
- Client DB schema (per-Space client DBs are managed by `server` at runtime via dynamic schema-evolving DDL — out of scope for this package).
...
Publish via private npm registry or GitHub Packages. Each runtime repo lists it as a dependency at a specific version. Resolved (F2 from earlier front design).
