...
The Drizzle schema for the master DB SHALL live in `web/src/db/schema/` and SHALL be the single source of truth for table definitions, types, and indexes.

#### Scenario: Back imports without redefining

- **WHEN** `server` or `server` imports table definitions
...
`web` SHALL publish the schema as an internal npm package `@baseout/db-schema` consumed by `server` at a pinned version.

#### Scenario: Version pinned by back

- **WHEN** `server`'s `server` updates Drizzle types
- **THEN** the back repo pins a specific `@baseout/db-schema` version and re-publishes via PR / lockfile change

### Requirement: Connection management

The master DB connection string SHALL live in Cloudflare Secrets. `web` SHALL connect via Drizzle for all reads/writes; `server` SHALL use its own Drizzle client and its own connection string from its Cloudflare Secrets.
...
- **THEN** front executes the write directly via Drizzle and no other repo inserts into these tables
