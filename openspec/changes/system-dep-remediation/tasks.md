...
- [~] 3.2 Verify: touched apps' `typecheck` + `build` + test suite green (baseline the known pre-existing failures — DO teardown hang, the 15 pre-existing web typecheck errors — do not chase). Commit locally with a §3.8 Verification block naming the cleared GHSA IDs. — _Verified 2026-08-05: auth unit suites **30/30 green** (auth-factory, session-cache, airtable/oauth); `astro check` = 20 errors ALL pre-existing `cloudflare:workers` workerd-typing baseline (0 auth-related, 0 new); `pnpm run build` **Complete!** (exit 0). **Local commit HELD for approval** per the human-tested loop._
