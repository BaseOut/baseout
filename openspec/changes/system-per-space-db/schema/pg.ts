 ...
 *
 * DESIGN-OF-RECORD for openspec/changes/system-per-space-db. This is a reference
 * copy that lives with the spec; on apply, move it to
 * packages/db-schema/src/space/pg.ts and wire a separate drizzle config
 * (PRD §21.1: separate configs for master DB vs client DBs). The SQLite/D1 mirror
 * is ./sqlite.ts — keep the two in lockstep (a parity test should enforce it).
 *
 * Conventions:
