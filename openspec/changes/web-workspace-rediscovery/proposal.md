...
The engine path documented in [`server-workspace-rediscovery`](../server-workspace-rediscovery/proposal.md) is invisible to the user without a frontend. The user-facing problem ("I added a base to my Airtable workspace and Baseout didn't notice") needs:
...
| Deferred to | Item |
|---|---|
| [`server-workspace-rediscovery`](../server-workspace-rediscovery/proposal.md) Phase 3 | Alarm-driven scheduled rediscovery banner refresh — the SSR-on-load read already covers it; explicit "poll for new events" deferred. |
...
- **Master DB**: no migrations from this change. Schema landed under [`server-workspace-rediscovery`](../server-workspace-rediscovery/proposal.md) Phase 1 (commit `3eeedfb`).
