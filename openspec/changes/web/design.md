...
`apps/web` (`@baseout/web`) is the Astro 6 SSR application that runs on Cloudflare Workers, port 4331 in dev. It owns the customer-facing UI surface and the `/api/*` endpoints that power it. After the port from `starter` HEAD (`29dfb5b`), it builds, type-checks, and runs end-to-end against a real Hyperdrive-fronted Postgres for auth, Airtable connections, and Stripe trial creation. Engine-coupled UI (the "Run Backup Now" form) is removed pending the rebuild of `apps/server`.
...
├── eslint.config.mjs
└── openspec → ../../openspec/changes/web/   (symlink, postinstall-managed)
```
...
- **`apps/server`** — does not exist yet. The previous direct HTTP wire is removed. When it lands, a sibling change reintroduces a backup-trigger path; per the existing `shared-client-isolation` proposal, browser → server traffic should route through `apps/web` proxies (not direct cross-origin), but that's a future-change choice.
...
| Change | Why deferred |
|---|---|
| `web-auth-extended` | password + 2FA TOTP + SAML — needs better-auth plugins + UI work |
| `shared-websocket-progress` | depends on `apps/server` durable object existing |
| `web-capability-api` | wrap the resolver lib in `/api/me/capabilities` + 5-min cache |
| `web-stripe-full` | webhooks + idempotency + plan upgrade + add-ons + credit packs |
| `web-byos-storage` | OAuth/IAM flows for Drive / Dropbox / Box / OneDrive / S3 / Frame.io |
| `web-ai-docs` | depends on `apps/server` (write path) and Cloudflare AI |
| `web-embedded` | iframe detection + postMessage framework |
| `web-migration-ux` | re-auth flow for migrated On2Air users |
| `web-data-views` | Data / Automations / Interfaces views |
| `web-notifications-ui` | dashboard panel + per-channel preferences |
| `web-restore-full` | snapshot picker + scope picker + post-restore verification |
| `web-schema-ui` | React Flow visualizer + changelog rendering |
| `web-onboarding-wizard` | full 5-step wizard + storage-destination flow |
