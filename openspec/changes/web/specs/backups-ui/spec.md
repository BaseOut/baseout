...
A "Run Now" button SHALL be present on the Backups page (and the dashboard). Clicking it SHALL POST to a `web` endpoint that writes a `backup_runs` row with `trigger_type='manual'` and calls `server`'s `/runs/{run_id}/start`.

#### Scenario: Run-now click

- **WHEN** a user clicks "Run Now" while no run is in progress
- **THEN** a `backup_runs` row is created and `server`'s `/runs/{run_id}/start` is invoked; the dashboard switches to live progress
...
The audit detail page SHALL render the `backup_runs` summary plus per-base detail from `backup_run_bases` and SHALL fetch the audit log content from a `server`-served endpoint when needed.

#### Scenario: Failed run detail

- **WHEN** a user opens detail for a failed run
- **THEN** the page renders the per-base failure reasons from `backup_run_bases` and surfaces a "see audit log" link to `server`'s read endpoint
...
- **THEN** `backup_configurations.frequency='weekly'` is persisted; `server`'s per-Space DO consumes it on the next scheduled cycle
