...
On submit, `web` SHALL write a `restore_runs` row with `status='pending'` and POST to `server`'s `/restores/{id}/start` with the service token.

#### Scenario: Submit succeeds

- **WHEN** the user submits a restore form
- **THEN** a `restore_runs` row is created and `server`'s `/restores/{id}/start` returns 200
...
For Pro+ Spaces, when a restore covers entities that cannot be auto-restored (Automations, Interfaces), the UI SHALL render the AI-prompt bundle returned from `server`'s `/spaces/{id}/restore-bundle/{run_id}` endpoint, with copy-to-clipboard and step-by-step instructions.
