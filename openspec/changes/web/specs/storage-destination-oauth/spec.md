...
`web` SHALL implement the OAuth/IAM auth flows for Google Drive, Dropbox, Box, OneDrive, S3 (Growth+), Frame.io (Growth+), and BYOS Custom (Pro+). The actual write logic to the destination is owned by `server`; front only handles auth + config persistence.

#### Scenario: Dropbox OAuth complete

- **WHEN** a user completes the Dropbox OAuth flow in the wizard
- **THEN** the encrypted tokens are persisted on `storage_destinations` and `server` can read them at backup time
