...
- [x] 1.4 Rsync `starter` HEAD (`29dfb5b`) into `apps/web/`, excluding `node_modules`, `dist`, `.git`, `.astro`, `.wrangler`, `test-results`, `.superpowers`
...
- [x] 1.6 Surgical strip of engine wiring:
  ...
  - [x] delete `plans/connect-to-server-engine.md`
...
- [ ] 1.9 `pnpm install` from repo root; verify no install errors
...
- [ ] 3.1 Password / 2FA TOTP / SAML auth → `web-auth-extended`
- [ ] 3.3 `GET /api/me/capabilities` HTTP endpoint + 5-min cache + `enforceCapability` middleware → `web-capability-api`
- [ ] 3.4 Stripe webhook receiver, idempotency table, dunning, plan upgrade/downgrade, add-ons, credit packs → `web-stripe-full`
- [ ] 3.5 BYOS storage destination OAuth (Drive / Dropbox / Box / OneDrive / S3 / Frame.io) → `web-byos-storage`
- [ ] 3.6 5-step onboarding wizard with `spaces.onboarding_step` resume → `web-onboarding-wizard`
- [ ] 3.7 Pre-registration schema visualization → `web-pre-reg-schema-viz`
- [ ] 3.8 Live WebSocket progress (depends on `apps/server`) → `shared-websocket-progress`
- [ ] 3.9 Run-Backup-Now wire reintroduced (depends on `apps/server`) → `web-run-now`
- [ ] 3.10 In-app notifications panel + preferences → `web-notifications-ui`
- [ ] 3.11 Schema UI (React Flow + changelog) → `web-schema-ui`
- [ ] 3.12 Restore UI (snapshot picker + scope picker + post-restore verification) → `web-restore-full`
- [ ] 3.13 Data / Automations / Interfaces views → `web-data-views`
- [ ] 3.14 AI-Assisted Documentation (depends on `apps/server`) → `web-ai-docs`
- [ ] 3.15 Embedded-extension mode (iframe + postMessage) → `web-embedded`
- [ ] 3.16 Migration UX (`has_migrated` re-auth flow) → `web-migration-ux`
- [ ] 3.17 Extract `backup_runs` schema to `packages/db-schema` → `db-schema-backup-runs`
...
- [ ] 4.5 After verification, archive `/Users/autumnshakespeare/dev/baseout/baseout-starter` and `server-engine` (mark read-only; do not delete until prod is on the monorepo)
