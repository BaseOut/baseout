...
- **Fan-out control is the core design problem.** A naive per-record sweep is O(records) API calls against Airtable's ~5 rps per-base budget (50k records ≈ hours). The design targets `recordMetadata=commentCount` on the existing record-listing pass so only records with `commentCount > 0` are considered — verified by a spike task before build; fallback strategies documented in the design.
- **Count-delta skip (added 2026-07-25, founder direction):** before fetching, the task POSTs the observed counts to the engine's new `comments-plan` route and fetches comments **only for records whose count changed** since the last successful capture; records the engine reports as dropped-to-zero are resolved by sending an empty `complete` capture (no fetch). Unchanged counts cost zero comments-endpoint calls. If the plan call fails, the task falls back to refreshing all observed commented records (pre-optimization behavior). Accepted blind spot documented in the server change: same-count modifications (delete+add pairs, comment edits) surface only when the count next changes.
