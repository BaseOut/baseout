...
Workflows-side counterpart to [`server-instant-webhook`](../server-instant-webhook/proposal.md). The server-side change owns the per-Space DO that coalesces Airtable webhook events, the gap-detection + fallback-to-full-base logic, cursor advancement on `airtable_webhooks`, and the webhook registration lifecycle. This change owns the Trigger.dev task that performs the incremental backup once the DO has decided what to back up.
