...
Workflows-side counterpart to [`server-retention-and-cleanup`](../server-retention-and-cleanup/proposal.md). The server-side change owns the `runCleanupPass` orchestration, master-DB schema mirrors, the manual-cleanup HTTP route, and the apps/web UI. This change owns the Trigger.dev v3 scheduled task that invokes `runCleanupPass` on an hourly cron from `apps/workflows/`.
