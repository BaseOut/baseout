...
- Spawn sibling `workflows-<topic>` changes for each in-flight `server-<topic>` change that adds a Trigger.dev task. The server-side change keeps the orchestration + DO + Worker bits; the workflows-side change owns the task definition + helpers + tests.
...
- **OpenSpec symlinks**: `apps/workflows/openspec` symlinks to `openspec/changes/workflows/`. `apps/server/openspec` continues to symlink to `openspec/changes/server/` (renamed from `server`).
- **Cross-change family rename**: `server` and every `server-*` in-flight change is renamed to `server[-…]` to match the app directory. Trigger.dev-touching changes get a `workflows-<topic>` sibling that owns the task-side bullets.
...
- Adding new task types beyond what's already proposed in the sibling `workflows-<topic>` changes.
