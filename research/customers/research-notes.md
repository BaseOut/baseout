...
- **The real gaps:** restore is **whole-base only** and always creates a **new base** ("The existing base cannot be overwritten") — no record/table/field-level restore; the restored base has **no revision history**; the new base gets a **new base ID and new share links** — record IDs ARE preserved on native restore (owner-verified 2026-07-11), so internal logic keyed to record IDs survives, but **every external dependency that hardcodes the base ID breaks** (Zapier/Make, webhooks, API integrations, Sync connections, embedded links — see T-12). Third-party restores (e.g. On2Air) do **NOT** preserve record IDs. Retention is plan-gated; and snapshots live **inside Airtable** — they cannot be exported off-platform, so they satisfy no external-copy requirement.
...
- **Automations and interfaces are NOT exportable via the REST API — verified current through June 2026** (API changelog Feb 2024→Jun 2026 adds nothing for them). Community confirmations: https://community.airtable.com/development-apis-11/access-automation-via-api-5307 · https://community.airtable.com/development-apis-11/how-can-i-work-with-interfaces-via-api-4230 · https://community.airtable.com/automations-8/export-and-import-automations-22755. **Consequence: no third-party tool can fully back up automations/interfaces from the REST API alone** — the parts of a base you can't rebuild from memory are the parts you can't export.
- ⚠️ **Update `[FOUNDER-REPORTED 2026-07-25 — verify against Airtable MCP docs before print]`: Airtable's MCP server now exposes automations and interfaces**, and Baseout can back them up automatically through MCP (no intake submission needed) for customers who enable MCP access. The REST-API ceiling above still holds; the print-safe formulation is now "not exportable via the REST API" — never claim automations/interfaces are categorically un-backupable. Intake remains the path for non-MCP customers and for data beyond MCP's reach.
...
### N-11 – N-16. Additions from the external hyperagent synthesis (2026-07-11)
*Source: `hyperagent-report.md` — an external desk-research synthesis. Items below are adopted for the report; each carries its verification state. Anything `[UNVERIFIED-EXTERNAL]` needs an independent check before print.*

- **N-11. App Sandbox — native staging exists now.** GA **October 2025**, Business/Enterprise Scale only: a temporary schema/automation copy selectively publishable to production; merge is imperfect as of early 2026 (linked-record conflicts manual, some automation changes don't merge). Below those tiers, manual base duplication remains the only staging — and **duplicating a base does not preserve record IDs**, so external systems keyed to record IDs break on every duplication. https://community.airtable.com/announcements-6/new-app-sandbox-generally-available-46608 `[OFFICIAL announcement — verify link + current state before print]`
- **N-12. Automation limits & silent-failure mechanics.** Run caps are a **hard monthly stop** (Free 100 · Team 25,000 · Business 100,000 · Enterprise ~500,000/custom) — automations simply stop at the cap. Failure notifications historically went to **exactly one person** (the last enabler); configurable recipients shipped only in **early 2026**. Plus: an undocumented recursion/loop cap that halts silently, timeouts that produce no failure email, and undo (Ctrl+Z) that can trigger automations with nothing in revision history to explain why. `[COMMUNITY/OFFICIAL mix — verify cap figures against current plan docs; notification threads: https://community.airtable.com/automations-8/automation-failed-notifications-are-not-sent-23330]`
- **N-13. Destructive updates by design.** The API `PUT` "performs a destructive update and clears all unincluded cell values"; the automation **Update Record** action replaces rather than appends — a blank source field silently wipes destination data on the next run. Two-way sync defaults to **source-wins**. `[OFFICIAL API docs + COMMUNITY — verify quotes before print]`
- **N-14. Governance gaps.** Field/table locks restrict *editing*, not *visibility* — any collaborator incl. read-only can export a full CSV. Ownership transfer happens at the **workspace** level and Team/Business customers have **no admin override** when a builder leaves (Enterprise Admin Panel can reassign; below that it's Airtable Support with no guarantee). The one rigorous public convention standard is BlueDot Impact's `airtable-standards` repo (naming, field-description metadata, soft-deprecation). `[THIRD-PARTY/OFFICIAL mix — verify the CSV-export claim and the BlueDot repo before print]`
- **N-15. Compliance ≠ your DR.** Airtable holds SOC 2 Type II / ISO 27001 / ISO 27701; HIPAA is Enterprise-Scale-only (Health Information Exhibit/BAA). **These certify Airtable-the-vendor's infrastructure — not the restore of any individual customer's base.** No customer-facing RTO/RPO SLA exists. Pairs with N-8 as the shared-responsibility one-two. `[OFFICIAL cert claims — verify current cert list on the trust page before print]`
- **N-16. The AI write-path.** Airtable runs an **official MCP server** (mcp.airtable.com, OAuth, prebuilt Claude/ChatGPT connectors) that **inherits the authenticated user's permissions**, with schema mutation on the near-term roadmap; **Field Agents** run continuously inside bases; **HyperDB** (GA May 2025) connects bases to 100M+ external records; Cobuilder (Jul 2024) generates apps from prompts. Net: a new, low-friction write path into production bases that most teams' audit tooling doesn't see. `[OFFICIAL feature claims — verify MCP URL/docs + dates against Airtable's live docs; complements N-9's timeline]`
...
| **ProBackup** | Daily backups + granular restore; 19+ SaaS apps, Airtable flagship | Plus $31 / Pro $47 / Premium $86 per mo ($25/$38/$69 annual, ~19–20% off); **storage-based** (10–2,500 GB), not flat-tiered; daily-only on all tiers (no hourly); no AI features; 7-day trial, no free tier | Active *(re-verified 2026-07-23)* |
| **AirBackups** | Daily full backups to Google Cloud Storage; preserves record IDs in exports; 5-yr exponential retention | $15/base/mo or $150/base/yr (**USD** — prior "EUR" note outdated); no free tier/trial | Active *(re-verified 2026-07-23)* |
| **Export My Base** | Free one-shot export of all tables (CSV/ZIP) | Free ("totally free to use") | Active — indie side project, maintenance level unclear *(verified 2026-07-23, exportmybase.com)* |
| **BackupTable** | Auto + manual table/base backups (historically: free manual + $6/mo auto to Drive/Dropbox) | — | **Defunct** — domain registration lapsed entirely (whois: no match, verified 2026-07-23) |
...
- **Stacksync** — real-time two-way Airtable⇄Postgres/MySQL, SOC 2 Type II. ~~Sales-led pricing~~ **Correction (2026-07-23): pricing is public** — Starter $1,000/mo, Pro $3,000/mo, Managed Pro $4,200/mo (all billed annually) + per-record overage; Enterprise custom. Firmly enterprise ($12K/yr minimum), no free tier/trial. https://www.stacksync.com/pricing `[OFFICIAL]`
- **Whalesync** — two-way sync. ⚠️ **Repriced — re-verified 2026-07-23:** the old Starter $99 / Operator $249 (20k records) / Professional $599 lineup is **gone**; now Personal $5/mo (250 records) · Starter $20/mo (500 records, **Postgres unlocks here**) · Plus $40–200/mo by record count (1K–50K; annual −20%) · Pro $200–1,600/mo (adds Salesforce). No free tier; 2-week trial. Record caps are tiny at the low end — SQL is cheap now, but only at demo scale. https://www.whalesync.com/pricing `[OFFICIAL]`
...
- **Takeaway (revised 2026-07-23):** ~~since Sequin died, "query Airtable in SQL" means $249+/mo, sales-led pricing, or DIY ELT~~ **Whalesync's repricing killed the old framing** — Postgres sync now starts at $20/mo. The surviving gap is *scale + purpose*: cheap tiers cap at 500–1K records (real Airtable estates need $120–200+/mo at Whalesync, $1,000/mo at Stacksync), and every option is **sync, not backup** — no snapshots, retention, point-in-time restore, or schema history. Baseout's SQL story should lead with "affordable at real estate sizes, and it's a governed backup underneath," not "the only affordable SQL." **Do not print the $249 claim** — it feeds the report draft and pricing briefing; both need the revised framing.
...
- **T-12.** Snapshot-restore product-idea thread (Jun 2022 → May 2024): restore creates a new base — "All 3rd-party tools need to be completely reconfigured all over again from scratch." https://community.airtable.com/legacy-product-ideas-75/restoring-a-snapshot-overwriting-the-current-base-43137 `[COMMUNITY]` ⚠️ **Correction (owner-verified 2026-07-11):** the thread's "new record IDs" claim is wrong for native restores — native snapshot restore **preserves record IDs**; what changes is the **base ID + share links** (breaking anything that hardcodes them). Third-party restores (On2Air etc.) do NOT preserve record IDs.
...
### T-15b. DevOps-specific indie tooling `[UNVERIFIED-EXTERNAL]`
Named in the hyperagent synthesis but **absent from our verified landscape — existence-check every one before any mention in print**: SafeBackup (backup + compliance layer, EU S3), AirDiff (schema change tracking), `airtable-devops` CLI (Squix), AppGrape Schema Visualizer, SyncHub data-model explorer, `airtable-typegen`. If real, they support the "early, mostly indie" tooling-market framing; if not, drop. (AirMap we have verified — T-4.)
...
### A-8. DORA 2024 additions + AI-agent governance backdrop (from the hyperagent synthesis)
- **DORA 2024** (10th edition): a **fifth metric — rework rate** — joined the four keys; cumulative reach now cited as 39,000+ professionals; headline finding: a 25% increase in AI adoption associated with ~1.5% *lower* delivery throughput and ~7.2% *lower* stability — AI amplifies process discipline or its absence. https://cloud.google.com/blog/products/devops-sre/announcing-the-2024-dora-report `[OFFICIAL — verify figures against the report PDF before print]`
- **Analytical framings to reuse (concepts, in our own words):** on a platform with no gates, minutes-long lead time signals *missing controls*, not elite performance; native-snapshot-only teams have "an RPO they cannot state" (cadence is activity-based); "a backup and a recoverable base are not the same thing."
- **AI-agent security stats** (CSA/Zenity 53% permission-exceedance + 47% incident rate; Gravitee ~48% unsecured; Kore.ai 79–82% autonomous consequential actions; ~74% of comms-agent deployments rolled back) — `[UNVERIFIED-EXTERNAL, vendor surveys, NOT Airtable-specific]`. Usable only as heavily-hedged backdrop for the AI chapter ("adjacent enterprise surveys suggest…"), each with its original source located first.
...
1. **Snapshots DO capture automations, interfaces, extensions, and comments** — never claim they don't. The true gaps: whole-base-only restore into a *new* base, no revision history in the restored base, plan-gated retention, cannot be exported off-platform.
1b. **Record IDs on restore — get the direction right** (owner-verified): native snapshot restore **preserves record IDs**; the **base ID + share links** change (breaking integrations that hardcode them). Never claim native restore changes record IDs. Third-party restores (On2Air etc.) DO produce new record IDs.
...
10. All pricing figures are as-published mid-2026 and change often — re-verify before print. Tool-landscape pricing (T-1, T-3) re-verified **2026-07-23**; verified spreadsheet: `research/pricing/competitor-landscape.csv`.
12. **The "SQL access starts at $249+/mo" claim is dead** (Whalesync repriced — Postgres at $20/mo since at least Jul 2026). Never print it. Stale copies of the claim exist in `customer-segments-workshop-1.md` (§ compliance segment) and possibly the report draft — fix on next edit of those docs. Use the revised T-3 takeaway instead.
...
12. **From the hyperagent synthesis, do not use without independent verification:** its scale figures ($11.6B "last valuation", 170% NDR — ours in N-9 are better sourced; keep ours), every `[UNVERIFIED-EXTERNAL]` tool in T-15b, the AI-agent security percentages in A-8, and its named-company anecdotes (wholesale distributor, car-subscription company). Its month-level AI dates are self-flagged as approximate.
