...
|---|---|---|
| **Base snapshots** | Point-in-time copies, automatic (roughly daily under heavy use) + manual; capture tables, records, views, automations, interfaces, extensions, comments | Retention: 2 weeks (Free), 1 yr (Team), 2 yrs (Business), 3 yrs (Enterprise Scale, extendable to 10). Restore is **whole-base only**, always into a **new base** with new record IDs and share links — every integration must be rewired [RESEARCH: T-12]. Restored bases lose revision history. Snapshots cannot be exported off-platform. |
