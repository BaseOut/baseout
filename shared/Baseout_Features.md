...
| **Action** | An opt-in operation that Baseout performs *against* a customer's data — the one place (besides Restore) that Baseout writes to Airtable (e.g. publishing internal notes as Airtable descriptions, filling missing descriptions, deleting unused fields, applying a naming convention). Lives in its own "Actions" section, kept separate from the read-only capabilities. | Agent, Workflow, Automation, Task | **V2** — surfaced in V1 only as a "Planned" landing (`web-actions`, promoted from ui-only@7502f81). Requires a **separate** write-scope Airtable token (`schema.bases:write` / `data.records:write`) — the read-only connection backups run on cannot run an Action. Restore is the other write exception. "Workflow"/"Automation" are reserved (Automation = the Airtable artifact above; Workflow = its alias-to-avoid); "Agent" is reserved for the *inbound* external agents that POST to the Baseout API (PRD §11). NOTE: the V1-vs-V2 status of "Schema Management Actions" is inconsistent across specs (Features §7.3 marks Pro+; PRD §10 / Appendix B mark V2) — flagged for a future reconcile, not resolved here. |
