...
| Set up Cloudflare Secrets for all environments | All | Stripe, encryption key, OAuth secrets, DB strings (Cloudflare Email Service needs no runtime secret — binding-auth) |
| Scaffold `baseout-ui` package | `baseout-ui` | Astro + Tailwind + DaisyUI; basic layout, button, input, modal, table components |
| ✅ Verify `mail.baseout.com` in Cloudflare Email Service dashboard | Infrastructure | DONE — DKIM/SPF/DMARC configured; Workers Paid plan required for outbound sends |
...
Build React Email templates for all V1 triggers (see §19.1). Send via the `EMAIL` Workers binding (Cloudflare Email Service) — see `src/lib/email/send.ts`.
...
- [ ] All V1 email templates functional via Cloudflare Email Service
