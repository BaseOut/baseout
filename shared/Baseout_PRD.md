...
| **Launch** (Static + Schema DB) | ⚠️ No record data | Baseout stores schema only (field names, types, table structure) in D1 — no record-level content |
...
| Method | Status | Notes |
|---|---|---|
| **Magic Link** | ✅ V1 launch — sole primary method | Passwordless — user enters email, receives a one-time sign-in link. Powered by `better-auth`'s `magicLink` plugin. Default token TTL 5 min, single use. Delivery via Cloudflare Email Service (see §19). |
| **Email + Password** | ❌ Removed pre-launch | Dropped in favor of passwordless. `accounts.password` column removed; `/forgot-password` and `/reset-password` deleted. |
| **SSO (SAML)** | ✅ V1 (Enterprise tier only) | better-auth SSO plugin. Required before Enterprise can be sold. Deferred until after magic-link launch. |
| **2FA (TOTP)** | ✅ V1 (pre-launch addition) | Deferred until after magic-link launch. Authenticator app support via better-auth 2FA plugin. |
...
| **Google OAuth** | ❓ Evaluate pre-launch | Deferred until after magic-link launch. better-auth supports it natively; decision before public launch. |
...
|---|---|---|
| **Transactional email** | [Cloudflare Email Service](https://developers.cloudflare.com/email-service/) | All system emails: magic links, audit reports, error notifications, trial expiry, upgrade prompts. Sent from the verified `mail.baseout.com` domain via the `EMAIL` Workers binding. React Email templates are a post-V1 follow-up (currently inline HTML + text fallbacks). |
| **Email list / marketing** | V2 / TBD | Provider decision deferred until marketing email is actually built. Cloudflare Email Service is transactional-focused. |
...
All email templates built with inline HTML + text fallbacks and sent via **[Cloudflare Email Service](https://developers.cloudflare.com/email-service/)** for V1 through the `EMAIL` Workers binding. The `sendEmail()` abstraction in `src/lib/email/send.ts` remains the single seam for outbound mail so the provider can be swapped in future without touching call sites. React Email scaffolding remains a post-V1 follow-up.

### 19.1 Template Inventory (V1)

| Template | Trigger | Notes |
|---|---|---|
| **Magic Link** | User requests sign-in or registers | Contains one-time link with expiry (5 min — better-auth default). Primary auth method in V1. |
...
| ~~Password Reset~~ | ~~User requests password reset~~ | Obsolete — password auth removed pre-launch (see §13.1). |

### 19.2 Sending Infrastructure

- **Transactional domain:** `mail.baseout.com` — verified in the Cloudflare Email Service dashboard (DKIM/SPF/DMARC configured). A separate domain will be chosen for marketing if/when V2 adds that surface.
- **V1 provider:** Cloudflare Email Service (Workers binding), accessed only through the `sendEmail()` abstraction in `src/lib/email/send.ts`
- **Dev mode:** `sendEmail()` detects `import.meta.env.DEV` and logs the rendered payload (including the magic-link URL) to the worker console instead of calling the `EMAIL` binding, so local magic-link flows work without external dependencies
- No runtime API key — auth is handled by the `EMAIL` binding itself. `EMAIL_FROM` (the verified sender header) is a `vars` entry in `wrangler.jsonc`; the `send_email` binding is declared in the wrangler config.
