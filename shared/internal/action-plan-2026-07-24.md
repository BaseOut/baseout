...
- [x] **Baseout PRD §2.9:** amend the collection-method table — Automations: "Manual (user-submitted via intake)" → "MCP capture + manual intake"; **add a Comments row** (REST API; tier from Dan's §2 decision). Resolve or re-flag the §2.9-vs-Features-§4.2 tier discrepancy while in there (already flagged in `server-automations-interfaces-docs`). → **DRAFTED 2026-07-27:** §2.9 amended (Automations + Interfaces → MCP capture + intake; Comments row added with the recommended tier ⚠-flagged; Views widening noted on the Schema row; Launch-vs-Growth discrepancy RE-FLAGGED inline, not resolved — Dan reconciles).
- [x] **Baseout Features:** capability-matrix + naming-dictionary entries for comment backup (canonical name needed before any code per §1) and the automations collection-method note. → **DRAFTED 2026-07-27:** §1 gains **Comment**, **Workspace (Airtable)**, and **Media Library** canonical terms; §6.3 table amended (Comments row ‡, MCP collection methods, Views note); §8.1 gains a Media Library row; Appendix B notes MCP capture; §17 gains open questions **18/19/20** (comments tier, media tier + backfill, `workspacesAndBases:read` scope decision) — all tier cells ⚠ **Dan to confirm**.
