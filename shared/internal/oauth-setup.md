...
| Magic link on `staging.console.baseout.dev` sends but clicking it errors (prod origins fine) | The staging preview host is **behind Cloudflare Access** (verified 2026-08-26: any GET 302s to `openside.cloudflareaccess.com`) — an email-client click without an Access session dies at the Access wall; additionally previews' `PUBLIC_AUTH_BASE_URL` is the apex (`console.baseout.dev`, currently serving PROD) so baseURL-anchored redirects can cross origins. Entangled with Dan's env-separation work — don't churn wrangler previews until that lands (2026-08-26 call decision). | §4 origin-remap note above; `previews` block in [apps/web/wrangler.jsonc.example](../../apps/web/wrangler.jsonc.example); Access policy in the Cloudflare dashboard (Dan) |
