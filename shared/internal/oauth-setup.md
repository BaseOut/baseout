...
| OAuth refresh cron flips Airtable Connection to `invalid` / Reconnect | **Status (2026-06-05):** hardened proactive refresh is **re-enabled** on `baseout-server-dev` (`*/15`, [oauth-refresh.ts](../../apps/server/src/lib/oauth-refresh.ts)). Cron **never** writes `status='invalid'`; decrypt/unknown errors → `pending_reauth`. No `status='refreshing'` claims; 14m active cooldown; 10m lookahead; self-heal `pending_reauth`/`invalid` after 1m; id-only persist after CAS miss when Airtable already rotated. **Kill-switch:** `OAUTH_REFRESH_CRON_DISABLED=true`. **Diagnose:** `wrangler tail --env dev` → `oauth_refresh` / `oauth_refresh_tick`; `decrypt_failed` = key drift (redeploy web+server via deploy scripts); `invalid_grant` = true revocation → one Reconnect. | [apps/server/src/lib/oauth-refresh.ts](../../apps/server/src/lib/oauth-refresh.ts), [apps/server/src/index.ts](../../apps/server/src/index.ts) |
| Airtable OAuth app still shows "connected" but Baseout `/integrations` shows Reconnect (`pending_reauth` or `invalid`) | **With cron off:** only [persist.ts](../../apps/web/src/lib/airtable/persist.ts) (Reconnect) or legacy cron damage changes `connections.status`. Access tokens still expire (~60m) but status can stay `active` until a backup/API fails — use **Reconnect** once. **Dev DB:** `node --env-file-if-exists=.env apps/web/scripts/diag-conns.mjs`. If `status='invalid'` but Airtable still accepts the stored refresh token, status is stale — Reconnect to rewrite tokens (do not run `diag-probe-airtable.mjs` on a row you intend to keep; it rotates the grant without persisting). | [apps/web/src/lib/airtable/persist.ts](../../apps/web/src/lib/airtable/persist.ts), [apps/web/scripts/diag-conns.mjs](../../apps/web/scripts/diag-conns.mjs) |
