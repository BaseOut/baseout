...
| OAuth refresh cron flips Airtable Connection to Reconnect (`pending_reauth`) | **Status (2026-06-10):** restored **baseline** cron from `093c276` (May 11 V1 + race fix + `date_trunc` CAS), hardened so the cron **never writes `status='invalid'`** — only `pending_reauth` (apps/web retires duplicate rows to `invalid`). **Single writer:** cron only — `POST /connections/:id/token` is ConnectionDO decrypt again (no lazy refresh). Missing Airtable `refresh_token` on a 200 reuses the submitted grant (matches web). **Diagnose:** `wrangler tail --env dev` → `oauth_refresh` / `oauth_refresh_tick`; `decrypt_failed` = key drift; `invalid_grant` = true revocation; `cas_lost` = Airtable rotated but DB persist missed. | [apps/server/src/lib/oauth-refresh.ts](../../apps/server/src/lib/oauth-refresh.ts), [apps/server/src/index.ts](../../apps/server/src/index.ts) |
| Airtable OAuth app still shows "connected" but Baseout `/integrations` shows Reconnect (`pending_reauth` or `invalid`) | Cron keeps tokens fresh; backup decrypts the ciphertext snapshot from run start — if access token expired before cron ran, backup may fail until cron catches up or user Reconnects. **Dev DB:** `node --env-file-if-exists=.env apps/web/scripts/diag-conns.mjs`. Do not run `diag-probe-airtable.mjs` on a row you intend to keep. | [apps/web/src/lib/airtable/persist.ts](../../apps/web/src/lib/airtable/persist.ts), [apps/web/scripts/diag-conns.mjs](../../apps/web/scripts/diag-conns.mjs) |
