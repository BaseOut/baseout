...
| OAuth refresh cron flips Airtable Connection to `invalid` / Reconnect | **Status (2026-06-05):** cron **kill-switched** on dev (`OAUTH_REFRESH_CRON_DISABLED=true`) after overnight disconnect when dev Postgres hit connection limit (`53300`) — cron rotated Airtable tokens but DB persist failed (`cas_lost`), leaving a stale grant → `invalid_grant` → Reconnect. Hardened orchestrator retries persist after rotation and marks `pending_reauth` immediately if persist still fails; self-heal skips retired `invalid` dupes. Re-enable only after `diag-conns.mjs` runs clean and pool headroom confirmed. **Diagnose:** `wrangler tail --env dev` → `oauth_refresh` / `oauth_refresh_tick`; `decrypt_failed` = key drift; `invalid_grant` = true revocation or stale grant after failed persist. | [apps/server/src/lib/oauth-refresh.ts](../../apps/server/src/lib/oauth-refresh.ts), [apps/server/src/index.ts](../../apps/server/src/index.ts) |
| Airtable OAuth app still shows "connected" but Baseout `/integrations` shows Reconnect (`pending_reauth` or `invalid`) | **With cron off:** backup/rescan lazy-refresh at `POST /api/internal/connections/:id/token` ([resolve-airtable-token.ts](../../apps/server/src/lib/connections/resolve-airtable-token.ts)) refreshes near-expiry tokens before backups; true revocation still needs Reconnect. **Dev DB:** `node --env-file-if-exists=.env apps/web/scripts/diag-conns.mjs`. Do not run `diag-probe-airtable.mjs` on a row you intend to keep. | [apps/server/src/pages/api/internal/connections/token.ts](../../apps/server/src/pages/api/internal/connections/token.ts), [apps/web/scripts/diag-conns.mjs](../../apps/web/scripts/diag-conns.mjs) |
