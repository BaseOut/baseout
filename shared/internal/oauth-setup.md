...
|-----------|-------------------------------------------------------|---------------------|--------------------------------------|
| local     | `http://baseout.local:4333`                           | (no deploy)         | `pnpm --filter @baseout/admin dev`   |
...
| Required URI (this branch)                                                                | Registered? | Owner of registration |
|-------------------------------------------------------------------------------------------|-------------|-----------------------|
| `https://baseout.local:4331/api/connections/storage/box/callback`                         | ✅ done (proved by completed consent + callback persist on 2026-07-02) | autumn (Box Developer Console) |
...
>
> ⚠️ **Do not register `https://staging.console.baseout.dev/...` as the product
> preview origin.** That host is only the Cloudflare name of a preview created
> with `--name staging`. Dan's preview origin is **`https://console.baseout.dev`**.
> Today the apex still serves production (Enable for includes Production) — flip
> it to **Preview only** before `preview:sync` of `PUBLIC_AUTH_BASE_URL` and
> before relying on apex magic-link/OAuth. Per-branch URLs
> (`<preview-name>.console.baseout.dev`) still exist as platform aliases.
...
- [x] Add `https://baseout.local:4331/api/connections/storage/box/callback` (verified working 2026-07-02)
...
| First login on a fresh DB: welcome form submits (`/api/onboarding/complete` 200) but the user lands back on a BLANK /welcome; every nav re-bounces there for ~5 min; reads as "button goes nowhere / cleared everything / can't log out"; re-clicking the email link or waiting fixes it (Dan+Autumn, 2026-08-26 call) | **better-auth `cookieCache` staleness** — the signed `session_data` cookie (Max-Age 300, minted at magic-link verify with `termsAcceptedAt: null`) is served VERBATIM by `getSession`, and the middleware onboarding gate trusts it. Onboarding invalidated the middleware's in-memory `SESSION_CACHE` but not this second cookie layer. Same mechanism family as the Box OAuth /login bounce. **Fixed 2026-08-26**: success responses of `/api/onboarding/complete` expire both `session_data` variants (`expiredSessionDataCookies` in `apps/web/src/lib/session-cache.ts`). Any FUTURE route that mutates a field the middleware gates on must do the same. | [apps/web/src/pages/api/onboarding/complete.ts](../../apps/web/src/pages/api/onboarding/complete.ts); [apps/web/src/lib/session-cache.ts](../../apps/web/src/lib/session-cache.ts); gate at `applyOnboardingGate` in [apps/web/src/middleware.ts](../../apps/web/src/middleware.ts) |
| Magic link on `staging.console.baseout.dev` sends but clicking it errors (prod origins fine) | The staging preview host is **behind Cloudflare Access** (re-verified 2026-08-27: any GET 302s to `openside.cloudflareaccess.com`) — an email-client click without an Access session dies at the Access wall. **2026-08-27 update (dashboard preview settings read via `wrangler preview settings --worker-name baseout-console --json`):** `PUBLIC_AUTH_BASE_URL` is now correctly `https://staging.console.baseout.dev` (Dan's mid-call fix landed), so link construction is right; the remaining failure is Access-only. Smoke procedure: pass Access in a browser FIRST (open any staging page), then request the link and click it in the SAME browser. **Also visible in the same read: staging previews' `BACKUP_ENGINE` service binding resolves to `baseout-server` — the PRODUCTION engine.** This is the KNOWN previews-beta limitation (service bindings from previews resolve to the production deployment of the target — see the ⚠ comment in `apps/web/wrangler.jsonc.example` previews block), NOT a config mistake, and it dissolves in Dan's account-separation rework: staging in its own Cloudflare account binds `baseout-server` and can only ever reach that account's server. Worker config changes ride the wrangler files (a deploy restates them over the dashboard) — never hand-edit dashboard settings. | §4 origin-remap note above; `previews` block in [apps/web/wrangler.jsonc.example](../../apps/web/wrangler.jsonc.example); Access policy + preview settings in the Cloudflare dashboard (Dan) |
| Airtable Connect errors for one specific user while other accounts connect fine (observed 2026-07: Dan's account maxed out, errors cleared on their own days later as old grants expired) | **Airtable-side per-user OAuth grant limit** — the user's Airtable account has too many active grants for our client_id (each dev/test reconnect consumes a slot; disconnecting in Baseout only flips our `connections` row, it does NOT free the Airtable grant slot). NOT our dead-connection handling. | Airtable account settings → third-party integrations: revoke stale Baseout grants by hand. Longer term: proactive revocation on disconnect + a friendly error — see [action-plan-2026-07-24.md §1](action-plan-2026-07-24.md) |
| Deployed staff console (`baseout-admin-dev`) loops on its "Sign in" page after a successful web login | Admin on `workers.dev` can NEVER receive web's better-auth session cookie: the cookie is host-only and `workers.dev` is on the [public suffix list](https://publicsuffix.org/), so a `Domain=` cookie spanning the two Workers is impossible. The deployed flow therefore rides a **handoff token**: web's `/login` (and the signed-in `/login` bounce) resolves an allowlisted admin returnTo to the parameter-less `/api/admin/handoff` (target = web's `vars.ADMIN_APP_URL`; a `?to=<encoded origin>` param was dropped 2026-07-13 — better-auth's verify endpoint double-decodes the callbackURL and its relative-path regex rejects the revealed `https://` with INVALID_CALLBACK_URL), which mints a 60s AES-GCM token; admin's `/auth/handoff` opens it and sets its own host-only `baseout_admin_session` cookie. A loop means the handoff chain is broken: web's `vars.ADMIN_APP_URL` missing/wrong (nothing allowlisted → `returnTo` silently dropped), or `ADMIN_HANDOFF_SECRET` missing/mismatched between `apps/web/.dev.vars` and `apps/admin/.dev.vars` (admin fails `undecryptable`), or the token expired (>60s between mint and landing). Local dev (`baseout.local:4331`/`4333`) shares the host and never uses the handoff. | [apps/web/src/pages/api/admin/handoff.ts](../../apps/web/src/pages/api/admin/handoff.ts); [apps/admin/src/pages/auth/handoff.ts](../../apps/admin/src/pages/auth/handoff.ts); `vars.ADMIN_APP_URL` in [apps/web/wrangler.jsonc.example](../../apps/web/wrangler.jsonc.example); openspec/changes/shared-admin-dev-deploy |
...
| OAuth Connect completes (the `storage_destinations` / `connections` row IS persisted) but the browser lands on `/login` and the user stays logged out — while their `sessions` row in the DB is still valid | The browser withheld or lost the `better-auth.session_token` cookie on the cross-site return from the provider, and the 5-minute `session_data` cookieCache (which had been carrying `getSession` during active browsing) expired during the consent detour — so the return hop had neither cookie. **Server side can be healthy** — verified end-to-end 2026-07-02 during the Box incident: cookie minted plain-name/`SameSite=Lax`/non-`Secure`, `PUBLIC_AUTH_BASE_URL` `--var` applied, DB-fallback `getSession` accepted every valid session token. Diagnose before touching code: (1) `GET /api/internal/test/auth-config` (E2E-gated; HMAC of the literal payload `auth-config` with `E2E_TEST_TOKEN` in `X-E2E-Test-Auth`) reports the resolved cookie mode; (2) replay a real session: sign `HMAC-SHA256(BETTER_AUTH_SECRET, sessions.token)` **standard base64 with padding** (better-call rejects base64url — signature must be 44 chars ending `=`) and GET `/api/auth/get-session` with `better-auth.session_token=<token>.<sig>`. ⚠️ probe gotcha: `.dev.vars` values may be quoted — strip quotes before HMAC'ing or every probe false-negatives (cost half a day on 2026-07-02). If the server checks out, the cause is the browser jar (Brave shields / privacy modes / eviction — same class as the 4d2ddfc note above). Mitigation shipped 2026-07-02: middleware bounces to `/login?returnTo=<original path>` and `/login` honors it as the magic-link callbackURL, so the failure costs one login instead of stranding the user; the connection itself was never lost. | [apps/web/src/pages/api/internal/test/auth-config.ts](../../apps/web/src/pages/api/internal/test/auth-config.ts); [apps/web/src/middleware.ts](../../apps/web/src/middleware.ts) `buildLoginRedirect`; [apps/web/src/lib/return-to.ts](../../apps/web/src/lib/return-to.ts) |
| Forced to log back in every ~7 days (pre-2026-07-10) / every 30 days (after), on the SAME host, despite daily activity and a valid, sliding `sessions` row in the DB | The sliding window only slid server-side. better-auth re-issues the `session_token` cookie with a fresh Max-Age exactly once per `updateAge` period — inside the `needsRefresh` branch of `getSession` — and for page loads the ONLY `getSession` call is the one in [apps/web/src/middleware.ts](../../apps/web/src/middleware.ts), which discarded the returned headers. The browser cookie therefore kept its login-time Max-Age and hard-died `session.expiresIn` after each login regardless of activity (7d before `21ce401`, 30d after — that fix stretched the death clock without stopping it). Root-caused 2026-07-14 by replaying a stale session against `get-session`: `expires_at` slid in-request, response carried only `session_data`. Two dead ends worth remembering: `advanced.cookies.session_token.attributes.maxAge` CANNOT override the death clock (better-auth spreads its computed `maxAge` after the configured attributes), and `session.cookieCache.refreshCache` is force-disabled when a `database` is configured. Fix: middleware calls `auth.api.getSession({ headers, returnHeaders: true })` and forwards `headers.getSetCookie()` onto every response it returns (`appendSetCookies`, unit-pinned in `middleware.test.ts`) — verified live: a page request bearing a >24h-stale session now returns `session_token` with `Max-Age≈2592000`. Diagnosis recipe (session-replay probe) is in the 2026-07-02 row above. | [apps/web/src/middleware.ts](../../apps/web/src/middleware.ts) `returnHeaders` + `appendSetCookies`; [apps/web/src/lib/auth-factory.ts](../../apps/web/src/lib/auth-factory.ts) `session.expiresIn/updateAge` |
| OAuth refresh cron flips Airtable Connection to Reconnect / Disconnected | **RESOLVED 2026-07-16.** The 2026-06-10 `cas_lost` reconnect loop (cron racing Airtable's refresh-token rotation via `modified_at` CAS) was fixed exactly as planned: Airtable refresh now flows through a single on-demand owner — `POST /connections/:id/token` behind ConnectionDO with an **opaque claim id** (`resolve-airtable-token.ts`), and the DO on-demand branch is wrapped in `blockConcurrencyWhile` so concurrent refreshers can't replay a consumed token. The `*/15` sweep is live again (`env.dev.triggers`) but is being migrated to a **daily idle keep-alive** keyed on the refresh token's 60-day expiry (`refresh_token_expires_at`) rather than hourly access-token churn — gated by `AIRTABLE_KEEPALIVE_MODE` (`sweep`\|`shadow`\|`keepalive`, default `sweep` = old behavior; rollback is the flag). A benign Airtable `409` ("recently refreshed") is now classified `transient`, not `invalid`. Dead connections auto-invalidate after a ~10-day `pending_reauth` grace (`connection-auto-invalidate` on the daily cron). See `openspec` `shared-oauth-token-lifecycle` (closed) + the keep-alive reshape. **Do not flip to `keepalive` in a real env until the token-health gauge has an on-call alert wired** (currently a manual probe). | [apps/server/src/lib/cron/oauth-refresh-deps.ts](../../apps/server/src/lib/cron/oauth-refresh-deps.ts), [apps/server/src/lib/cron/keepalive-mode.ts](../../apps/server/src/lib/cron/keepalive-mode.ts), [apps/server/wrangler.jsonc.example](../../apps/server/wrangler.jsonc.example) |
